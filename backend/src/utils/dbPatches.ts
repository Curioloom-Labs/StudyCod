...
type SqlQuery = (sql: string, parameters?: unknown[]) => Promise<unknown>;
type PatchExecutor = QueryRunner | SqlQuery;
type DbPatchContext = { query: SqlQuery; queryRunner?: QueryRunner; strict: boolean };
export const LEGACY_DB_PATCH_PHASES = [
  "identity-task",
  "theory-support",
  "library-execution",
  "contest-certificate",
...
export type LegacyDbPatchPhase = typeof LEGACY_DB_PATCH_PHASES[number];

const appQuery: SqlQuery = (sql, parameters) => AppDataSource.query(sql, parameters);
const patchContext = new AsyncLocalStorage<DbPatchContext>();

function getMetadataName(sql: string, parameters: unknown[] | undefined, pattern: RegExp): string | null {
...
async function query(sql: string, parameters?: unknown[]): Promise<unknown> {
  const context = patchContext.getStore();
  const queryRunner = context?.queryRunner;
  if (!queryRunner) return (context?.query ?? appQuery)(sql, parameters);

  // TypeORM's schema API is the source of truth for metadata checks during
  // tracked migrations. The legacy helpers still issue portable DDL/data SQL,
  // but never use SHOW queries against a separate connection or transaction.
  const normalized = sql.trim().replace(/\s+/g, " ");
  if (/^SHOW TABLES LIKE /i.test(normalized)) {
    const tableName = getMetadataName(normalized, parameters, /^SHOW TABLES LIKE ['"]([^'"]+)['"]/i);
    return tableName && await queryRunner.hasTable(tableName) ? [{ [`Tables_in_${tableName}`]: tableName }] : [];
  }
  if (/^SHOW COLUMNS FROM /i.test(normalized)) {
    const tableMatch = normalized.match(/^SHOW COLUMNS FROM [`"]([^`"]+)[`"] LIKE /i);
    const tableName = tableMatch?.[1] ?? null;
    const columnName = getMetadataName(normalized, parameters, /LIKE ['"]([^'"]+)['"]/i);
    if (!tableName || !columnName) return [];
    const table = await queryRunner.getTable(tableName);
    const column = table?.findColumnByName(columnName);
    if (!column) return [];
    const columnType = Array.isArray(column.enum) && column.enum.length > 0
      ? `enum(${column.enum.map(value => `'${String(value).replace(/'/g, "''")}'`).join(",")})`
      : String(column.type);
    return [{
      Field: column.name,
      Type: columnType,
      ...
      Extra: column.isGenerated ? "auto_increment" : "",
    }];
  }
  if (/^SHOW INDEX FROM /i.test(normalized)) {
    const tableMatch = normalized.match(/^SHOW INDEX FROM [`"]([^`"]+)[`"]/i);
    const tableName = tableMatch?.[1] ?? null;
    const indexName = normalized.match(/Key_name\s*=\s*['"]([^'"]+)['"]/i)?.[1]
      ?? (typeof parameters?.[0] === "string" ? parameters[0] : null);
    if (!tableName || !indexName) return [];
    const table = await queryRunner.getTable(tableName);
    const exists = Boolean(table?.indices.some(index => index.name === indexName)
      || table?.uniques.some(index => index.name === indexName));
    return exists ? [{ Key_name: indexName }] : [];
  }
  return context.query(sql, parameters);
}

function createPatchContext(executor: PatchExecutor): DbPatchContext {
  if (typeof executor === "function") return { query: executor, strict: true };
  return { query: executor.query.bind(executor) as SqlQuery, queryRunner: executor, strict: true };
}

function sqlErrorProperty(error: unknown, key: string): unknown {
  return error && typeof error === "object"
    ? (error as Record<string, unknown>)[key]
    : undefined;
}

// Legacy helpers historically swallowed database errors. Keep the emergency
// entry point backward-compatible, but make the tracked migration fail fast so
// TypeORM cannot record a partially applied schema repair as successful.
const logger = {
  ...baseLogger,
  error: (message: string, meta?: unknown): never | void => {
    baseLogger.error(message, meta);
    if (!patchContext.getStore()?.strict) return;
    const details = meta && typeof meta === "object"
      ? String((meta as Record<string, unknown>).message ?? "")
      : "";
    throw new Error(`DB_PATCH_FAILED: ${message}${details ? ` (${details})` : ""}`);
  },
};
...
// the codebase. The implementation is now reached by five tracked phase
// migrations through the QueryRunner executor; the guarded public entry point
// remains only as an explicitly opt-in emergency compatibility path. New code
// MUST use TypeORM migrations. To intentionally run these legacy patches in an
 ...
 *
 * Tracked migrations must not go through this guarded entry point — they use
 * the phase-level QueryRunner executor below.
 */
export async function applyDbPatches(): Promise<void> {
  const enabled = String(env.DB_PATCHES_ENABLED ?? "").trim() === "1";
  if (!enabled) {
    throw new Error(
...
/**
 * UNGUARDED body — exported only for the tracked migration phase files and
 * the guarded emergency wrapper. Do not call from server runtime code. The
 * leading underscore is a soft signal — `applyDbPatches` above is the public
 * surface.
 *
 ...
 */
export async function _legacyDbPatchPhaseForMigration(
  phase: LegacyDbPatchPhase,
  queryExecutor: PatchExecutor = appQuery,
): Promise<void> {
  return patchContext.run(createPatchContext(queryExecutor), async () => {
    try {
      await runDbPatchPhase(phase);
    } finally {
      // The AsyncLocalStorage context is scoped to this invocation, so a
      // concurrent migration or emergency one-shot cannot inherit its query
      // runner or strict error policy.
    }
  });
}

export async function _legacyDbPatchesForMigration(queryExecutor: PatchExecutor = appQuery): Promise<void> {
  return patchContext.run(createPatchContext(queryExecutor), async () => {
    try {
      for (const phase of LEGACY_DB_PATCH_PHASES) {
        await runDbPatchPhase(phase);
      }
    } finally {
      // The AsyncLocalStorage context is scoped to this invocation, so a
...
async function runDbPatchPhase(phase: LegacyDbPatchPhase): Promise<void> {
  switch (phase) {
    case "identity-task":
      await applyIdentityAndTaskPatches();
      return;
    case "theory-support":
      await applyTheoryAndSupportPatches();
      return;
    case "library-execution":
      await applyLibraryAndExecutionPatches();
      return;
    case "contest-certificate":
      await applyContestAndCertificatePatches();
      return;
    case "topic-theory":
      await applyTopicTheoryPatches();
      return;
  }
}

async function applyIdentityAndTaskPatches(): Promise<void> {
...
async function applyTheoryAndSupportPatches(): Promise<void> {
...
async function applyLibraryAndExecutionPatches(): Promise<void> {
...
async function applyContestAndCertificatePatches(): Promise<void> {
  await ensureContestTables();
  await ensureContestSubmissionsGroupScoresColumn();
  await ensureCertificateTables();
}

async function applyTopicTheoryPatches(): Promise<void> {
...
async function ensureUsersDifusColumns(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'users'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const ensureColumn = async (columnName: string, sql: string): Promise<void> => {
      const col = (await query("SHOW COLUMNS FROM `users` LIKE ?", [columnName])) as Array<SqlRow>;
    ...
    const ensureDecimalColumn = async (columnName: string): Promise<void> => {
      ...
      await query(`ALTER TABLE \`users\` MODIFY COLUMN \`${columnName}\` DECIMAL(6,3) NOT NULL DEFAULT 0`);
    ...
    logger.error("[DB Patch] Failed to ensure users difus columns:", {
      message: sqlErrorProperty(err, "message"),
      code: sqlErrorProperty(err, "code"),
      errno: sqlErrorProperty(err, "errno"),
      sqlState: sqlErrorProperty(err, "sqlState")
    });
  }
}
...
async function ensureContestCertificateSettingsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'contest_certificate_settings'")) as Array<SqlRow>;
...
async function ensureCertificateTemplatesTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'certificate_templates'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureCertificateFieldsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'certificate_fields'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureCertificatesTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'certificates'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureCertificateJobQueueTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'certificate_job_queue'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureContestsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'contests'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) {
      const col = (await query("SHOW COLUMNS FROM `contests` LIKE 'allow_upsolve'")) as Array<SqlRow>;
      if (!Array.isArray(col) || col.length === 0) {
        logger.warn("[DB Patch] Column contests.allow_upsolve is missing. Adding...");
        await query("ALTER TABLE `contests` ADD COLUMN allow_upsolve TINYINT(1) NOT NULL DEFAULT 1");
        logger.info("[DB Patch] Added column contests.allow_upsolve");
      }

      const scoringCol = (await query("SHOW COLUMNS FROM `contests` LIKE 'scoring_mode'")) as Array<SqlRow>;
...
async function ensureContestProblemsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'contest_problems'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) {
      const pointsCol = (await query("SHOW COLUMNS FROM `contest_problems` LIKE 'points'")) as Array<SqlRow>;
      if (!Array.isArray(pointsCol) || pointsCol.length === 0) {
        logger.warn("[DB Patch] Column contest_problems.points is missing. Adding...");
        await query("ALTER TABLE `contest_problems` ADD COLUMN points INT NULL AFTER label");
...
async function ensureContestParticipantsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'contest_participants'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) {
      const dqCol = (await query("SHOW COLUMNS FROM `contest_participants` LIKE 'is_disqualified'")) as Array<SqlRow>;
      if (!Array.isArray(dqCol) || dqCol.length === 0) {
        logger.warn("[DB Patch] Column contest_participants.is_disqualified is missing. Adding...");
        await query("ALTER TABLE `contest_participants` ADD COLUMN is_disqualified TINYINT(1) NOT NULL DEFAULT 0 AFTER display_name");
        logger.info("[DB Patch] Added column contest_participants.is_disqualified");
      }

      const reasonCol = (await query("SHOW COLUMNS FROM `contest_participants` LIKE 'disqualification_reason'")) as Array<SqlRow>;
      if (!Array.isArray(reasonCol) || reasonCol.length === 0) {
        ...
        await query("ALTER TABLE `contest_participants` ADD COLUMN disqualification_reason TEXT NULL AFTER is_disqualified");
        logger.info("[DB Patch] Added column contest_participants.disqualification_reason");
      }

      const atCol = (await query("SHOW COLUMNS FROM `contest_participants` LIKE 'disqualified_at'")) as Array<SqlRow>;
      if (!Array.isArray(atCol) || atCol.length === 0) {
        logger.warn("[DB Patch] Column contest_participants.disqualified_at is missing. Adding...");
        await query("ALTER TABLE `contest_participants` ADD COLUMN disqualified_at DATETIME NULL AFTER disqualification_reason");
        logger.info("[DB Patch] Added column contest_participants.disqualified_at");
      }

      const accountHandleCol = (await query("SHOW COLUMNS FROM `contest_participants` LIKE 'contest_account_handle'")) as Array<SqlRow>;
      if (!Array.isArray(accountHandleCol) || accountHandleCol.length === 0) {
        ...
        await query("ALTER TABLE `contest_participants` ADD COLUMN contest_account_handle VARCHAR(120) NULL AFTER is_disqualified");
        logger.info("[DB Patch] Added column contest_participants.contest_account_handle");
      }

      const accountNoteCol = (await query("SHOW COLUMNS FROM `contest_participants` LIKE 'contest_account_note'")) as Array<SqlRow>;
      if (!Array.isArray(accountNoteCol) || accountNoteCol.length === 0) {
        ...
        await query("ALTER TABLE `contest_participants` ADD COLUMN contest_account_note VARCHAR(255) NULL AFTER contest_account_handle");
...
async function ensureContestSubmissionsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'contest_submissions'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) {
      const col = (await query("SHOW COLUMNS FROM `contest_submissions` LIKE 'phase'")) as Array<SqlRow>;
      if (!Array.isArray(col) || col.length === 0) {
        logger.warn("[DB Patch] Column contest_submissions.phase is missing. Adding...");
        await query("ALTER TABLE `contest_submissions` ADD COLUMN phase ENUM('CONTEST','UPSOLVE') NOT NULL DEFAULT 'CONTEST'");
...
async function ensureEnumColumnHasValues(params: {
  ...
  try {
    const tables = (await query(`SHOW TABLES LIKE ?`, [params.table])) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const col = (await query(`SHOW COLUMNS FROM \`${params.table}\` LIKE ?`, [params.column])) as Array<SqlRow>;
    if (!Array.isArray(col) || col.length === 0) return;
    ...
    await query(`ALTER TABLE \`${params.table}\` MODIFY COLUMN \`${params.column}\` ${enumSql} ${nullSql} ${defaultSql}`);
...
async function ensureClassesGradingSystemColumn(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'classes'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `classes` LIKE 'grading_system'")) as Array<SqlRow>;
    if (!Array.isArray(col) || col.length === 0) {
...
async function ensureTargetedAssignmentsColumns(): Promise<void> {
  try {
    const ensureJsonColumn = async (table: string, column: string): Promise<void> => {
      const tableRows = (await query(`SHOW TABLES LIKE '${table}'`)) as Array<SqlRow>;
      if (!Array.isArray(tableRows) || tableRows.length === 0) return;

      const col = (await query(`SHOW COLUMNS FROM \`${table}\` LIKE ?`, [column])) as Array<SqlRow>;
      if (Array.isArray(col) && col.length > 0) return;

      logger.warn(`[DB Patch] Column ${table}.${column} is missing. Applying ALTER TABLE...`);
      await query(`ALTER TABLE \`${table}\` ADD COLUMN \`${column}\` JSON NULL`);
...
async function ensureTheoryBlocksContentColumn(): Promise<void> {
  ...
  try {
    const tables = (await query("SHOW TABLES LIKE 'theory_blocks'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `theory_blocks` LIKE 'content'")) as Array<SqlRow>;
    const type = String(col?.[0]?.Type ?? "").toLowerCase();
    if (!type) return;
    ...
    await query("ALTER TABLE `theory_blocks` MODIFY COLUMN `content` MEDIUMTEXT NOT NULL");
...
async function ensureTopicsTheoryMarkdownColumn(): Promise<void> {
  ...
  try {
    const tables = (await query("SHOW TABLES LIKE 'topics'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `topics` LIKE 'theory_markdown'")) as Array<SqlRow>;
    if (!Array.isArray(col) || col.length === 0) return;
...
async function dropTheoryBlockRevisionsTableIfExists(): Promise<void> {
  ...
  try {
    const rows = (await query("SHOW TABLES LIKE 'theory_block_revisions'")) as Array<SqlRow>;
    if (!Array.isArray(rows) || rows.length === 0) return;

    logger.warn("[DB Patch] Dropping table theory_block_revisions (obsolete)...");
    await query("DROP TABLE `theory_block_revisions`");
...
async function ensureLibraryTaskRevisionsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'library_task_revisions'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureLibraryTaskRevisionsSnapshotColumn(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'library_task_revisions'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `library_task_revisions` LIKE 'snapshot'")) as Array<SqlRow>;
    if (!Array.isArray(col) || col.length === 0) return;
...
async function ensureLibraryTaskAttemptsMultiLangColumns(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'library_task_attempts'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const hasColumn = async (name: string): Promise<boolean> => {
      const col = (await query("SHOW COLUMNS FROM `library_task_attempts` LIKE ?", [name])) as Array<SqlRow>;
    ...
    if (!(await hasColumn("draft_code_by_language"))) {
      ...
      await query("ALTER TABLE `library_task_attempts` ADD COLUMN `draft_code_by_language` MEDIUMTEXT NULL AFTER `draft_code`");
    ...
    if (!(await hasColumn("last_submitted_code_by_language"))) {
      ...
      await query("ALTER TABLE `library_task_attempts` ADD COLUMN `last_submitted_code_by_language` MEDIUMTEXT NULL AFTER `last_submitted_code`");
...
async function ensureLibraryTasksOjColumns(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'library_tasks'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const hasColumn = async (name: string): Promise<boolean> => {
      const col = (await query("SHOW COLUMNS FROM `library_tasks` LIKE ?", [name])) as Array<SqlRow>;
      return Array.isArray(col) && col.length > 0;
    };
    ...
    try {
      const rows = (await query("SELECT id, problem_code FROM `library_tasks` ORDER BY id ASC")) as Array<{
        id: number;
      ...
      for (const r of rows || []) {
        ...
        const raw = String(r.problem_code ?? "").trim();
        ...
        used.add(candidate);
        if (candidate !== raw) {
          await query("UPDATE `library_tasks` SET `problem_code` = ? WHERE `id` = ?", [candidate, id]);
          changed += 1;
        }
      }

      await query("UPDATE `library_tasks` SET `slug` = CONCAT('task-', `id`) WHERE (`slug` IS NULL OR `slug` = '')");
      if (changed > 0) {
      ...
      logger.warn("[DB Patch] Failed to backfill/sanitize library_tasks problem_code/slug", { message: sqlErrorProperty(e, "message"), code: sqlErrorProperty(e, "code") });
    }

    // Indexes (best-effort; ignore errors for duplicate names, etc.)
    try {
      const rows = (await query("SHOW INDEX FROM `library_tasks` WHERE Key_name = 'uq_library_tasks_problem_code'")) as Array<SqlRow>;
      if (!Array.isArray(rows) || rows.length === 0) {
        await query("ALTER TABLE `library_tasks` ADD UNIQUE KEY `uq_library_tasks_problem_code` (`problem_code`)");
        logger.info("[DB Patch] Added unique index uq_library_tasks_problem_code");
      }
    } catch (e: unknown) {
      logger.warn("[DB Patch] Failed to ensure unique index uq_library_tasks_problem_code", { message: sqlErrorProperty(e, "message"), code: sqlErrorProperty(e, "code") });
    }

    try {
      const rows = (await query("SHOW INDEX FROM `library_tasks` WHERE Key_name = 'idx_library_tasks_slug'")) as Array<SqlRow>;
      if (!Array.isArray(rows) || rows.length === 0) {
        await query("CREATE INDEX `idx_library_tasks_slug` ON `library_tasks` (`slug`)");
        logger.info("[DB Patch] Added index idx_library_tasks_slug");
      }
    } catch (e: unknown) {
      logger.warn("[DB Patch] Failed to ensure index idx_library_tasks_slug", { message: sqlErrorProperty(e, "message"), code: sqlErrorProperty(e, "code") });
    }
  } catch (err: unknown) {
...
async function ensureTestDataTextColumns(): Promise<void> {
  ...
  try {
    const tables = (await query("SHOW TABLES LIKE 'test_data'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const inputCol = (await query("SHOW COLUMNS FROM `test_data` LIKE 'input'")) as Array<SqlRow>;
    const expectedCol = (await query("SHOW COLUMNS FROM `test_data` LIKE 'expected_output'")) as Array<SqlRow>;
...
async function ensureLibraryTaskAttemptsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'library_task_attempts'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureLibraryTasksTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'library_tasks'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureTaskTheoriesLibraryTaskColumn(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'task_theories'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `task_theories` LIKE 'library_task_id'")) as Array<SqlRow>;
    if (Array.isArray(col) && col.length > 0) return;

    logger.warn("[DB Patch] Column task_theories.library_task_id is missing. Applying ALTER TABLE...");
    try {
      await query("ALTER TABLE `task_theories` ADD COLUMN `library_task_id` INT NULL AFTER `edu_task_id`");
      await query("ALTER TABLE `task_theories` ADD UNIQUE KEY `uq_task_theories_library_task_id` (`library_task_id`)");
      await query("ALTER TABLE `task_theories` ADD CONSTRAINT `fk_task_theories_library_task` FOREIGN KEY (`library_task_id`) REFERENCES `library_tasks`(`id`) ON DELETE CASCADE");
      ...
      logger.warn("[DB Patch] Failed to add task_theories.library_task_id (may already exist / FK name conflict):", {
        message: sqlErrorProperty(e, "message"),
        code: sqlErrorProperty(e, "code")
      });
    }
  } catch (err: unknown) {
...
async function ensureTestDataLibraryTaskColumn(): Promise<void> {
  try {
    ...
    const col = (await query("SHOW COLUMNS FROM `test_data` LIKE 'library_task_id'")) as Array<SqlRow>;
    if (Array.isArray(col) && col.length > 0) return;

    logger.warn("[DB Patch] Column test_data.library_task_id is missing. Applying ALTER TABLE...");
    try {
      await query("ALTER TABLE `test_data` ADD COLUMN `library_task_id` INT NULL AFTER `personal_task_id`");
      await query("ALTER TABLE `test_data` ADD INDEX `idx_test_data_library_task_id` (`library_task_id`)");
      await query("ALTER TABLE `test_data` ADD CONSTRAINT `fk_test_data_library_task` FOREIGN KEY (`library_task_id`) REFERENCES `library_tasks`(`id`) ON DELETE CASCADE");
...
async function ensureUsersPlacementColumns(): Promise<void> {
  try {
    ...
    const doneCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_done'")) as Array<SqlRow>;
    if (!Array.isArray(doneCol) || doneCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_done` TINYINT(1) NOT NULL DEFAULT 0");
      logger.info("[DB Patch] Added column users.placement_done");
    }

    const levelCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_level'")) as Array<SqlRow>;
    if (!Array.isArray(levelCol) || levelCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_level` ENUM('BEGINNER','INTERMEDIATE','ADVANCED') NULL DEFAULT NULL");
      logger.info("[DB Patch] Added column users.placement_level");
    }

    const scoreCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_score'")) as Array<SqlRow>;
    if (!Array.isArray(scoreCol) || scoreCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_score` INT NULL DEFAULT NULL");
      logger.info("[DB Patch] Added column users.placement_score");
    }

    const masteredJavaCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_mastered_until_topic_index_java'")) as Array<SqlRow>;
    if (!Array.isArray(masteredJavaCol) || masteredJavaCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_mastered_until_topic_index_java` INT NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_mastered_until_topic_index_java");
    }

    const masteredPythonCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_mastered_until_topic_index_python'")) as Array<SqlRow>;
    if (!Array.isArray(masteredPythonCol) || masteredPythonCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_mastered_until_topic_index_python` INT NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_mastered_until_topic_index_python");
    }

    const doneAtCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_done_at'")) as Array<SqlRow>;
    if (!Array.isArray(doneAtCol) || doneAtCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_done_at` DATETIME(6) NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_done_at");
    }

    const codingPassedCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_coding_passed'")) as Array<SqlRow>;
    if (!Array.isArray(codingPassedCol) || codingPassedCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_coding_passed` TINYINT(1) NOT NULL DEFAULT 0");
      console.log("[DB Patch] Added column users.placement_coding_passed");
    }

    const codingLevelCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_coding_level'")) as Array<SqlRow>;
    if (!Array.isArray(codingLevelCol) || codingLevelCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_coding_level` ENUM('BEGINNER','INTERMEDIATE','ADVANCED') NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_coding_level");
    }

    const codingTaskIdCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_coding_task_id'")) as Array<SqlRow>;
    if (!Array.isArray(codingTaskIdCol) || codingTaskIdCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_coding_task_id` VARCHAR(80) NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_coding_task_id");
    }

    const codingScoreCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_coding_score'")) as Array<SqlRow>;
    if (!Array.isArray(codingScoreCol) || codingScoreCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_coding_score` INT NULL DEFAULT NULL");
      console.log("[DB Patch] Added column users.placement_coding_score");
    }

    const codingDoneAtCol = (await query("SHOW COLUMNS FROM `users` LIKE 'placement_coding_done_at'")) as Array<SqlRow>;
    if (!Array.isArray(codingDoneAtCol) || codingDoneAtCol.length === 0) {
      ...
      await query("ALTER TABLE `users` ADD COLUMN `placement_coding_done_at` DATETIME(6) NULL DEFAULT NULL");
...
async function ensureMarketingEmailsEnabledColumns(): Promise<void> {
  try {
    const ensure = async (table: "users" | "students"): Promise<void> => {
      ...
      const col = (await query(`SHOW COLUMNS FROM \`${table}\` LIKE 'marketing_emails_enabled'`)) as Array<SqlRow>;
      if (Array.isArray(col) && col.length > 0) return;

      logger.warn(`[DB Patch] Column ${table}.marketing_emails_enabled is missing. Applying ALTER TABLE...`);
      await query(`ALTER TABLE \`${table}\` ADD COLUMN \`marketing_emails_enabled\` TINYINT(1) NOT NULL DEFAULT 1`);

      // Best-effort backfill for rows inserted before the column existed.
      try {
        await query(`UPDATE \`${table}\` SET \`marketing_emails_enabled\` = 1 WHERE \`marketing_emails_enabled\` IS NULL`);
      } catch {
...
async function ensureStudentsUiLanguageColumn(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'students'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `students` LIKE 'ui_language'")) as Array<SqlRow>;
    if (!Array.isArray(col) || col.length === 0) {
      logger.warn("[DB Patch] Column students.ui_language is missing. Applying ALTER TABLE...");
      await query("ALTER TABLE `students` ADD COLUMN `ui_language` ENUM('uk','en') NOT NULL DEFAULT 'en' AFTER `email`");
...
async function ensureUsersBirthdayGreetedYearColumn(): Promise<void> {
  try {
    ...
    const col = (await query("SHOW COLUMNS FROM `users` LIKE 'birthday_greeted_year'")) as Array<SqlRow>;
    if (Array.isArray(col) && col.length > 0) return;

    logger.warn("[DB Patch] Column users.birthday_greeted_year is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `users` ADD COLUMN `birthday_greeted_year` INT NULL DEFAULT NULL");
...
async function ensureTasksIoTypeColumn(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'tasks'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const col = (await query("SHOW COLUMNS FROM `tasks` LIKE 'io_type'")) as Array<SqlRow>;
    if (Array.isArray(col) && col.length > 0) return;

    logger.warn("[DB Patch] Column tasks.io_type is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `tasks` ADD COLUMN `io_type` ENUM('STDIN_STDOUT','NO_INPUT_FIXED_OUTPUT','NO_INPUT_FREE_OUTPUT') NOT NULL DEFAULT 'STDIN_STDOUT'");

    // Backfill for legacy rows (best-effort; default covers it, but keep it explicit).
    try {
      await query("UPDATE `tasks` SET `io_type` = 'STDIN_STDOUT' WHERE `io_type` IS NULL OR `io_type` = ''");
    } catch {
      // ignore
    }
  ...
  const ensureForTable = async (table: string): Promise<void> => {
    try {
      ...
      const hasColumn = async (columnName: string): Promise<boolean> => {
        const col = (await query(`SHOW COLUMNS FROM \`${table}\` LIKE ?`, [columnName])) as Array<SqlRow>;
        return Array.isArray(col) && col.length > 0;
      };

      if (!(await hasColumn("task_mode"))) {
        logger.warn(`[DB Patch] Column ${table}.task_mode is missing. Applying ALTER TABLE...`);
        await query(`ALTER TABLE \`${table}\` ADD COLUMN \`task_mode\` ENUM('CODE','WEB') NOT NULL DEFAULT 'CODE' AFTER \`template\``);
      ...
      if (!(await hasColumn("web_template_files"))) {
        ...
        await query(`ALTER TABLE \`${table}\` ADD COLUMN \`web_template_files\` MEDIUMTEXT NULL AFTER \`task_mode\``);
      ...
      if (!(await hasColumn("web_validation_rules"))) {
        ...
        await query(`ALTER TABLE \`${table}\` ADD COLUMN \`web_validation_rules\` MEDIUMTEXT NULL AFTER \`web_template_files\``);
      ...
      if (!(await hasColumn("web_validation_profile"))) {
        ...
        await query(`ALTER TABLE \`${table}\` ADD COLUMN \`web_validation_profile\` MEDIUMTEXT NULL AFTER \`web_validation_rules\``);
      ...
      if ((table === "tasks" || table === "topic_tasks" || table === "edu_tasks" || table === "library_tasks") && !(await hasColumn("project_spec"))) {
        ...
        await query(`ALTER TABLE \`${table}\` ADD COLUMN \`project_spec\` MEDIUMTEXT NULL AFTER \`task_mode\``);
...
async function ensureEduGradesScoringColumns(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'edu_grades'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const scoreCol = (await query("SHOW COLUMNS FROM `edu_grades` LIKE 'score'")) as Array<SqlRow>;
    if (!Array.isArray(scoreCol) || scoreCol.length === 0) {
      console.warn("[DB Patch] Column edu_grades.score is missing. Applying ALTER TABLE...");
      await query("ALTER TABLE `edu_grades` ADD COLUMN `score` INT NULL");
      console.log("[DB Patch] Added column edu_grades.score");
    }

    const maxScoreCol = (await query("SHOW COLUMNS FROM `edu_grades` LIKE 'max_score'")) as Array<SqlRow>;
    if (!Array.isArray(maxScoreCol) || maxScoreCol.length === 0) {
      ...
      await query("ALTER TABLE `edu_grades` ADD COLUMN `max_score` INT NULL");
      console.log("[DB Patch] Added column edu_grades.max_score");
    }

    const groupScoresCol = (await query("SHOW COLUMNS FROM `edu_grades` LIKE 'group_scores'")) as Array<SqlRow>;
    if (!Array.isArray(groupScoresCol) || groupScoresCol.length === 0) {
      ...
      await query("ALTER TABLE `edu_grades` ADD COLUMN `group_scores` TEXT NULL");
...
async function ensureEduPerformanceIndexes(): Promise<void> {
  const hasTable = async (table: string): Promise<boolean> => {
    const rows = (await query(`SHOW TABLES LIKE '${table}'`)) as Array<SqlRow>;
  ...
  const hasIndex = async (table: string, indexName: string): Promise<boolean> => {
    ...
    const rows = (await query(`SHOW INDEX FROM \`${table}\` WHERE Key_name = '${indexName}'`)) as Array<SqlRow>;
    return Array.isArray(rows) && rows.length > 0;
  };
...
async function ensureMaintenanceStateTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'maintenance_state'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureMaintenanceStateSingletonRow(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'maintenance_state'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;
    const rows = (await query("SELECT id FROM maintenance_state WHERE id = 1 LIMIT 1")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
    console.warn("[DB Patch] maintenance_state singleton row is missing. Inserting id=1...");
    await query("INSERT INTO maintenance_state (id, enabled, title, message, until, created_at, updated_at) VALUES (1, 0, 'Технічне обслуговування', 'Ми оновлюємо платформу.', NULL, NOW(6), NOW(6))");
...
async function ensureSupportTicketsTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'support_tickets'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureSupportChatTables(): Promise<void> {
  try {
    const convRows = (await query("SHOW TABLES LIKE 'support_conversations'")) as Array<SqlRow>;
    ...
    const msgRows = (await query("SHOW TABLES LIKE 'support_messages'")) as Array<SqlRow>;
    ...
    const attRows = (await query("SHOW TABLES LIKE 'support_attachments'")) as Array<SqlRow>;
    ...
    const convStudentCol = (await query("SHOW COLUMNS FROM `support_conversations` LIKE 'student_id'")) as Array<SqlRow>;
    if (!Array.isArray(convStudentCol) || convStudentCol.length === 0) {
      ...
      try {
        await query("ALTER TABLE `support_conversations` ADD COLUMN `student_id` INT NULL AFTER `user_id`");
        await query("ALTER TABLE `support_conversations` ADD INDEX `idx_support_conversations_student_id` (`student_id`)");
        await query("ALTER TABLE `support_conversations` ADD CONSTRAINT `fk_support_conversations_student` FOREIGN KEY (`student_id`) REFERENCES `students`(`id`) ON DELETE SET NULL");
    ...
    const msgStudentCol = (await query("SHOW COLUMNS FROM `support_messages` LIKE 'sender_student_id'")) as Array<SqlRow>;
    if (!Array.isArray(msgStudentCol) || msgStudentCol.length === 0) {
      ...
      try {
        await query("ALTER TABLE `support_messages` ADD COLUMN `sender_student_id` INT NULL AFTER `sender_user_id`");
        await query("ALTER TABLE `support_messages` ADD CONSTRAINT `fk_support_messages_sender_student` FOREIGN KEY (`sender_student_id`) REFERENCES `students`(`id`) ON DELETE SET NULL");
...
async function ensureGradeAppealsTables(): Promise<void> {
  try {
    const appealRows = (await query("SHOW TABLES LIKE 'grade_appeals'")) as Array<SqlRow>;
    ...
    const messageRows = (await query("SHOW TABLES LIKE 'grade_appeal_messages'")) as Array<SqlRow>;
    ...
    const lastMessageCol = (await query("SHOW COLUMNS FROM `grade_appeals` LIKE 'last_message_at'")) as Array<SqlRow>;
    if (!Array.isArray(lastMessageCol) || lastMessageCol.length === 0) {
      ...
      await query("ALTER TABLE `grade_appeals` ADD COLUMN `last_message_at` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) AFTER `resolved_at`");
      await query("ALTER TABLE `grade_appeals` ADD INDEX `idx_grade_appeals_last_message_at` (`last_message_at`)");
...
async function ensureEduHintFeedbackTable(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'edu_hint_feedback'")) as Array<SqlRow>;
    ...
    const hasColumn = async (columnName: string): Promise<boolean> => {
      const rows = (await query("SHOW COLUMNS FROM `edu_hint_feedback` LIKE ?", [columnName])) as Array<SqlRow>;
      return Array.isArray(rows) && rows.length > 0;
    };
...
async function ensureTeacherDigestDeliveriesTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'teacher_digest_deliveries'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function migrateLegacySupportTicketsToChatIfNeeded(): Promise<void> {
  try {
    const tables = (await query("SHOW TABLES LIKE 'support_tickets'")) as Array<SqlRow>;
    const convTables = (await query("SHOW TABLES LIKE 'support_conversations'")) as Array<SqlRow>;
    const msgTables = (await query("SHOW TABLES LIKE 'support_messages'")) as Array<SqlRow>;
    if (!Array.isArray(tables) || tables.length === 0) return;
    if (!Array.isArray(convTables) || convTables.length === 0) return;
    if (!Array.isArray(msgTables) || msgTables.length === 0) return;

    const existing = (await query("SELECT id FROM support_conversations LIMIT 1")) as Array<SqlRow>;
    if (Array.isArray(existing) && existing.length > 0) return;

    const tickets = (await query("SELECT id, user_email, subject, message, status, created_at, answered_at FROM support_tickets ORDER BY created_at ASC")) as Array<SqlRow>;
    ...
    for (const t of tickets) {
      const insertConv: unknown = await query(
      ...
      const conversationId = Number(sqlErrorProperty(insertConv, "insertId"));
      ...
      if (String(t.status) === "ANSWERED" && t.answered_at) {
        ...
        await query("UPDATE support_conversations SET last_message_at = ? WHERE id = ?", [t.answered_at, conversationId]);
      }
    }
...
async function normalizeAndSanitizeTheoryBlocks(): Promise<void> {
  try {
    const exists = (await query("SHOW TABLES LIKE 'theory_blocks'")) as Array<SqlRow>;
    if (!Array.isArray(exists) || exists.length === 0) return;
    const rows = (await query("SELECT id, title, content FROM theory_blocks")) as Array<{
      id: number;
      title: string;
      content: string;
    }>;
    ...
    for (const r of rows) {
      ...
      if (next !== original) {
        await query("UPDATE theory_blocks SET content = ?, updated_at = NOW(6) WHERE id = ?", [next, r.id]);
        changed++;
      }
    }
    if (changed > 0) {
...
async function ensureTestDataIsHiddenColumn(): Promise<void> {
  try {
    const rows = (await query("SHOW COLUMNS FROM `test_data` LIKE 'is_hidden'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) {
      return;
    }
    console.warn("[DB Patch] Column test_data.is_hidden is missing. Applying ALTER TABLE to add it...");
    await query("ALTER TABLE `test_data` ADD COLUMN `is_hidden` TINYINT(1) NOT NULL DEFAULT 0");
...
async function ensureContestSubmissionsGroupScoresColumn(): Promise<void> {
  try {
    const tableRows = (await query("SHOW TABLES LIKE 'contest_submissions'")) as Array<SqlRow>;
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const rows = (await query("SHOW COLUMNS FROM `contest_submissions` LIKE 'group_scores'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;

    logger.warn("[DB Patch] Column contest_submissions.group_scores is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `contest_submissions` ADD COLUMN `group_scores` TEXT NULL AFTER `compile_error_kind`");
...
async function ensureTestDataKindColumn(): Promise<void> {
  try {
    ...
    const rows = (await query("SHOW COLUMNS FROM `test_data` LIKE 'kind'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;

    logger.warn("[DB Patch] Column test_data.kind is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `test_data` ADD COLUMN `kind` ENUM('SAMPLE','JUDGE') NOT NULL DEFAULT 'JUDGE' AFTER `is_hidden`");
    await query("ALTER TABLE `test_data` ADD INDEX `idx_test_data_kind` (`kind`)");
...
async function ensureTestDataSubtaskColumn(): Promise<void> {
  try {
    ...
    const rows = (await query("SHOW COLUMNS FROM `test_data` LIKE 'subtask'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;

    logger.warn("[DB Patch] Column test_data.subtask is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `test_data` ADD COLUMN `subtask` VARCHAR(64) NULL AFTER `points`");
...
async function ensureTestDataSourceColumn(): Promise<void> {
  try {
    ...
    const rows = (await query("SHOW COLUMNS FROM `test_data` LIKE 'source'")) as Array<SqlRow>;
    if (!Array.isArray(rows) || rows.length === 0) {
      logger.warn("[DB Patch] Column test_data.source is missing. Applying ALTER TABLE...");
      await query("ALTER TABLE `test_data` ADD COLUMN `source` ENUM('MANUAL','AI_GENERATED','LIBRARY_IMPORTED') NOT NULL DEFAULT 'MANUAL' AFTER `kind`");
      await query("ALTER TABLE `test_data` ADD INDEX `idx_test_data_source` (`source`)");
...
async function ensureTestDataPaginationIndexes(): Promise<void> {
  try {
    ...
    const hasIndex = async (name: string): Promise<boolean> => {
      const rows = (await query("SHOW INDEX FROM `test_data` WHERE Key_name = ?", [name])) as Array<SqlRow>;
      return Array.isArray(rows) && rows.length > 0;
    };
...
async function backfillTestDataKindFromIsHidden(): Promise<void> {
  try {
    ...
    const kindCol = (await query("SHOW COLUMNS FROM `test_data` LIKE 'kind'")) as Array<SqlRow>;
...
async function ensureTheoryBlocksTable(): Promise<void> {
  try {
    const rows = (await query("SHOW TABLES LIKE 'theory_blocks'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
...
async function ensureTopicsTheoryBlockIdColumn(): Promise<void> {
  try {
    const rows = (await query("SHOW COLUMNS FROM `topics` LIKE 'theory_block_id'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
    console.warn("[DB Patch] Column topics.theory_block_id is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `topics` ADD COLUMN `theory_block_id` INT NULL");
...
async function ensureTopicsNewTheoryBlockIdColumn(): Promise<void> {
  try {
    const rows = (await query("SHOW COLUMNS FROM `topics_new` LIKE 'theory_block_id'")) as Array<SqlRow>;
    if (Array.isArray(rows) && rows.length > 0) return;
    console.warn("[DB Patch] Column topics_new.theory_block_id is missing. Applying ALTER TABLE...");
    await query("ALTER TABLE `topics_new` ADD COLUMN `theory_block_id` INT NULL");
...
async function migrateLegacyTopicTheoryMarkdownToTheoryBlocks(): Promise<void> {
  try {
    const legacyCol = (await query("SHOW COLUMNS FROM `topics` LIKE 'theory_markdown'")) as Array<SqlRow>;
    if (!Array.isArray(legacyCol) || legacyCol.length === 0) return;
    const topics = (await query("SELECT id, title, theory_markdown FROM topics WHERE (theory_block_id IS NULL OR theory_block_id = 0) AND theory_markdown IS NOT NULL AND TRIM(theory_markdown) <> ''")) as Array<{
      id: number;
      title: string;
      theory_markdown: string;
    }>;
    if (!Array.isArray(topics) || topics.length === 0) return;
    const existingBlocks = (await query("SELECT id, content FROM theory_blocks")) as Array<{
      id: number;
      content: string;
    }>;
    ...
    for (const t of topics) {
      ...
      if (!blockId) {
        const insertRes: unknown = await query("INSERT INTO theory_blocks (title, content, version, level, tags, created_at, updated_at) VALUES (?, ?, 1, NULL, NULL, NOW(6), NOW(6))", [String(t.title ?? "Theory"), content]);
        blockId = Number(sqlErrorProperty(insertRes, "insertId"));
        if (blockId) byHash.set(h, blockId);
      }
      if (blockId) {
        await query("UPDATE topics SET theory_block_id = ? WHERE id = ?", [blockId, t.id]);
      }
    }
...
async function fixIntroPythonFixedSumTaskTests(): Promise<void> {
  try {
    const candidates = (await query("SELECT id, title, description, lang FROM tasks WHERE lang='PYTHON' AND title LIKE '%Вступ до Python та інтерпретатора%';")) as Array<{
      id: number;
      title: string;
      description: string;
      lang: string;
    ...
    for (const taskId of taskIdsToFix) {
      await query("DELETE FROM test_data WHERE personal_task_id = ?", [taskId]);
      await query("INSERT INTO test_data (input, expected_output, is_hidden, points, created_at, personal_task_id) VALUES (?, ?, 0, 100, NOW(), ?)", ["", "8", taskId]);
    }
...
async function fixNoInputFixedExampleTaskTests(): Promise<void> {
  try {
    const rows = (await query("SELECT id, title, description FROM tasks WHERE lang='PYTHON' AND description IS NOT NULL AND TRIM(description) <> ''")) as Array<{
      id: number;
      title: string;
      description: string;
    }>;
    ...
    for (const c of candidates) {
      await query("DELETE FROM test_data WHERE personal_task_id = ?", [c.id]);
      await query("INSERT INTO test_data (input, expected_output, is_hidden, points, created_at, personal_task_id) VALUES (?, ?, 0, 100, NOW(), ?)", ["", c.expected, c.id]);
    }
