...
- Виведи рівно всі рядки результату, визначені умовою, у правильному порядку; не пропускай рядок для нуля. Не округлюй і не відкидай дробову частину, якщо це явно не вимагає умова.
- Для кожного тесту підстав input у формулу з умови й окремо звір кожен рядок output, перш ніж повертати JSON.
...
МАТРИЦЯ ПОКРИТТЯ (склади мовчки перед JSON):
- Спочатку визнач окремі класи поведінки з повної умови: різний тест — це інша гілка/правило, а не просто інше число.
- Покрий кожну дозволену гілку, потім додай мінімум, значення біля кожного порогу, звичайне середнє значення та default/невідоме значення, якщо це дозволено.
- Для кількох входів змінюй взаємозв’язок і порядок значень. Не генеруй послідовний ряд 1, 2, 3 або косметичні варіації, що потрапляють в ту саму гілку.
- Кожен тест має мати окрему причину існування в цій матриці. Якщо допустимих унікальних input менше, ніж потрібно, не вигадуй дублікати — така задача потребує багатшого домену.
