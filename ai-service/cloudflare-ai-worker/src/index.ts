...
function buildSystemPrompt(mode: string, lang: Language): string {
  ...
  const modeRule: Record<string, string> = {
    "generate-task": lang === "en"
      ? "Create a full judgeable programming task in JSON. The task must be solvable by ONE COMPLETE PROGRAM FILE (main function/entry point) using stdin/stdout OR no input with fixed output. Keep the learner-facing practicalTask compact: usually 2-4 connected sentences, roughly 45% shorter than a verbose explanation. A small olympiad-style story is welcome when the real task is embedded clearly in it. Include context, given data, required behaviour, exact output, and only the constraints needed to solve it. Describe WHAT the program should do, never HOW to implement it. Do not reveal a solution technique: never say to use if/else, switch, loops, recursion, sorting, binary search, or any named algorithm/construct. Do not add meta-instructions such as 'Ensure your program handles...', a checklist, theory, or a solution explanation. For branching topics, describe domain rules and outcomes only. Avoid a shallow task that is just three obvious values mapped to three messages; provide varied valid input and non-obvious behavioural cases without increasing algorithmic complexity. Every valid input must produce deterministic non-empty stdout; if no result exists, require an explicit sentinel such as NONE, 0, or NO_MATCH instead of a blank line. Return 1-2 public examples in examples, each with exact input and output that obey the same IO contract; examples are for understanding, not solution hints. ABSOLUTELY FORBIDDEN: ask to implement a standalone function/method/class; ask to write just a function body; ask to create files/folders/projects; ask to configure IDE/compiler/build tools; ask for unit tests or test functions. The ONLY acceptable solution format is a complete, runnable program that reads input and produces deterministic output."
      : "Створи повну перевірювану задачу з програмування у форматі JSON. Завдання має бути розв'язаним ОДНИМ ПОВНИМ ФАЙЛОМ ПРОГРАМИ (main функція/точка входу) через stdin/stdout АБО без вводу з фіксованим виводом. Поле practicalTask для учня зроби компактним: зазвичай 2–4 зв'язні речення, приблизно на 45% коротші за розлоге пояснення. Допустима невелика олімпіадна історія, якщо в ній чітко заховане саме завдання. Додай контекст, вхідні дані, потрібну поведінку, точний вивід і лише необхідні обмеження. Описуй ЩО повинна робити програма, але не ЯК це реалізувати. Не розкривай спосіб розв'язання: ніколи не кажи використати if/else, switch, цикли, рекурсію, сортування, бінарний пошук чи будь-який названий алгоритм/конструкцію. Не додавай мета-інструкцій на кшталт «переконайтеся, що програма обробляє...», чекліста, теорії чи пояснення рішення. Для тем про розгалуження описуй лише правила предметної області та результати. Не створюй поверхневу задачу, що зводиться до трьох очевидних значень і трьох повідомлень; дай різноманітні допустимі вводи й неочевидні поведінкові випадки без ускладнення алгоритму. Для кожного допустимого вводу stdout має містити детермінований непорожній результат; якщо результату немає, задай явний sentinel на кшталт NONE, 0 або NO_MATCH замість порожнього рядка. Поверни 1–2 публічні приклади в examples з точними input та output, що відповідають тому самому IO-контракту; приклади потрібні для розуміння, а не для підказки рішення. АБСОЛЮТНО ЗАБОРОНЕНО: просити реалізувати окремо функцію/метод/клас; просити писати лише тіло функції; просити створювати файли/папки/проєкти; просити налаштовувати IDE/компілятор/інструменти збірки; просити unit-тести. ЄДИНИЙ прийнятний формат розв'язку — повна, запустима програма, яка читає вхідні дані і виробляє детермінований вивід.",
    "generate-task-condition": lang === "en"
      ? "Generate only one standalone judgeable programming task statement in 3-5 connected sentences. Include context, given data, required behaviour, exact output, and constraints. Do not include examples, markdown metadata, theory, commentary, solution steps, or implementation hints. Never tell the learner to use if/else, switch, loops, recursion, sorting, binary search, or another named technique. Describe domain rules and observable outcomes only. Do not use meta-instructions such as 'Ensure your program handles...'."
      : "Згенеруй лише одну самостійну перевірювану умову задачі у 3–5 зв'язних реченнях. Додай контекст, дані, потрібну поведінку, точний вивід і обмеження. Не додавай прикладів, markdown-метаданих, теорії, зайвих пояснень, кроків рішення чи підказок щодо реалізації. Ніколи не кажи учневі використовувати if/else, switch, цикли, рекурсію, сортування, бінарний пошук чи іншу названу техніку. Описуй лише правила предметної області та спостережуваний результат. Не використовуй мета-інструкції на кшталт «переконайтеся, що програма обробляє...».",
