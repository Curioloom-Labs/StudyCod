...
function buildSystemPrompt(mode: string, lang: Language): string {
  ...
  const modeRule: Record<string, string> = {
    "generate-task": lang === "en"
      ? "Create a full judgeable programming task in JSON. The task must be solvable by ONE COMPLETE PROGRAM FILE (main function/entry point) using stdin/stdout OR no input with fixed output. The learner-facing practicalTask must be a connected 3-5 sentence problem statement: context, given data, required behaviour, exact output, and necessary constraints. It must describe WHAT the program should do, never HOW to implement it. Do not reveal a solution technique: never say to use if/else, switch, loops, recursion, sorting, binary search, or any named algorithm/construct. Do not add meta-instructions such as 'Ensure your program handles...' or a solution checklist. For branching topics, describe domain rules and outcomes only. Avoid a shallow task that is just three obvious values mapped to three messages; provide enough valid input variety for meaningful behavioural tests without increasing algorithmic complexity. Every valid input must produce deterministic non-empty stdout; if no result exists, require an explicit sentinel such as NONE, 0, or NO_MATCH instead of a blank line. ABSOLUTELY FORBIDDEN: ask to implement a standalone function/method/class; ask to write just a function body; ask to create files/folders/projects; ask to configure IDE/compiler/build tools; ask for unit tests or test functions. The ONLY acceptable solution format is a complete, runnable program that reads input and produces deterministic output."
      : "Створи повну перевірювану задачу з програмування у форматі JSON. Завдання має бути розв'язаним ОДНИМ ПОВНИМ ФАЙЛОМ ПРОГРАМИ (main функція/точка входу) через stdin/stdout АБО без вводу з фіксованим виводом. Поле practicalTask для учня має бути зв'язною умовою з 3–5 речень: контекст, дані, потрібна поведінка, точний вивід і необхідні обмеження. Описуй ЩО повинна робити програма, але не ЯК це реалізувати. Не розкривай спосіб розв'язання: ніколи не кажи використати if/else, switch, цикли, рекурсію, сортування, бінарний пошук чи будь-який названий алгоритм/конструкцію. Не додавай мета-інструкцій на кшталт «переконайтеся, що програма обробляє...», чекліста чи розбору рішення. Для тем про розгалуження описуй лише правила предметної області та результати. Не створюй поверхневу задачу, що зводиться до трьох очевидних значень і трьох повідомлень; дай достатньо допустимих варіантів вводу для змістовних поведінкових тестів без ускладнення алгоритму. Для кожного допустимого вводу stdout має містити детермінований непорожній результат; якщо результату немає, задай явний sentinel на кшталт NONE, 0 або NO_MATCH замість порожнього рядка. АБСОЛЮТНО ЗАБОРОНЕНО: просити реалізувати окремо функцію/метод/клас; просити писати лише тіло функції; просити створювати файли/папки/проєкти; просити налаштовувати IDE/компілятор/інструменти збірки; просити unit-тести. ЄДИНИЙ прийнятний формат розв'язку — повна, запустима програма, яка читає вхідні дані і виробляє детермінований вивід.",
