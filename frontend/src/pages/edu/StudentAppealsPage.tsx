      ...
      <div className="max-w-7xl mx-auto space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-mono text-text-primary">{tr("Мої апеляції", "My appeals")}</h1>
          </div>
          ...
          <div className="lg:col-span-5 space-y-3">
            {appeals.length === 0 ? (
              <Card className="p-6 text-center text-text-secondary">
                <div className="font-mono text-sm">{tr("Поки немає апеляцій", "No appeals yet")}</div>
                <div className="text-xs mt-2">{tr("Створіть апеляцію для перевірки оцінки", "Create an appeal to review a grade")}</div>
              </Card>
            ) : (
              appeals.map(item => (
                <Card
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="min-w-0">
                      <div className="text-sm font-mono text-text-primary truncate">#{item.id} • {item.targetTitle}</div>
                      <div className="text-xs text-text-muted mt-1">{item.targetType === "SUMMARY_GRADE" ? tr("Підсумкова", "Summary") : tr("Завдання", "Task")}</div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10px] px-2 py-1 border border-border text-text-secondary whitespace-nowrap">
            ...
            {!selectedAppealId ? (
              ...
              <Card className="p-4 sm:p-5 space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-mono text-text-primary">#{selectedAppeal.id} • {selectedAppeal.targetTitle}</h2>
                    <div className="text-xs text-text-muted mt-1">{formatDateTime(selectedAppeal.createdAt)}</div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="border border-border p-3">
                    <div className="text-text-muted mb-1">{tr("Причина", "Reason")}</div>
                    <div className="text-text-primary">{formatReasonCode(selectedAppeal.reasonCode)}</div>
                  </div>
                  <div className="border border-border p-3">
                    <div className="text-text-muted mb-1">{tr("Оцінка", "Grade")}</div>
                ...
                <div className="space-y-2">
                  <div className="text-xs text-text-muted">{tr("Ваш опис", "Your description")}</div>
                ...
                {selectedAppeal.resolutionText ? (
                  <div className="space-y-2">
                    <div className="text-xs text-text-muted">{tr("Рішення вчителя", "Teacher resolution")}</div>
                ...
                <div className="pt-1">
                  ...
                  <div className="max-h-[320px] overflow-y-auto border border-border p-3 space-y-2 bg-bg-code">
                    {messages.length === 0 ? (
