    ...
    <div className="mx-auto max-w-[1280px] px-4 py-8 sm:px-6 lg:px-10 lg:py-12">
      <header className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          ...
          {editingName ? <div className="mt-3 flex flex-wrap gap-2"><input autoFocus value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} className="rounded-xl border border-[#19291d]/12 bg-white px-4 py-2 text-2xl font-bold tracking-[-.04em] outline-none ring-[#00ff88]/25 focus:ring-4 dark:border-white/10 dark:bg-[#111b14] dark:text-white" /><button type="button" disabled={busy} onClick={() => void updateName()} className="rounded-xl bg-[#00d978] px-4 py-2 text-sm font-bold text-[#062211]">Зберегти</button><button type="button" onClick={() => { setEditingName(false); setNameDraft(active?.name ?? ""); }} className="rounded-xl px-3 text-sm font-bold text-[#6d7d72]">Скасувати</button></div> : <h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-bold tracking-[-.055em] text-[#17241b] dark:text-[#eff5f0] sm:text-5xl">{active?.name || "Твоя команда"}<button type="button" onClick={() => setEditingName(true)} className="ml-3 inline-grid h-8 w-8 translate-y-[-3px] place-items-center rounded-lg text-[#799087] transition hover:bg-[#edf3ed] dark:hover:bg-white/[.07]"><Pencil className="h-4 w-4" /></button></h1>}
        ...
        <button type="button" onClick={() => setShowInvite(true)} className="rounded-xl bg-[#153321] px-4 py-3 text-sm font-bold text-white shadow-[0_12px_28px_rgba(20,67,40,.18)] transition hover:-translate-y-0.5 dark:bg-[#00d978] dark:text-[#062211]"><UserPlus className="mr-2 inline h-4 w-4" />Запросити людину</button>
      </header>
      {error && <div role="alert" className="mb-5 rounded-2xl border border-[#ff6b9d]/25 bg-[#ff6b9d]/[.08] px-4 py-3 text-sm font-medium text-[#c4436b] dark:text-[#ff9abd]">{error}</div>}
      {orgs.length === 0 ? <section className="rounded-[28px] border border-dashed border-[#19291d]/16 px-7 py-20 text-center dark:border-white/10"><Building2 className="mx-auto h-9 w-9 text-[#ff9b2e]" /><h2 className="mt-4 text-2xl font-bold">Ще немає простору команди</h2><p className="mx-auto mt-3 max-w-md text-base leading-7 text-[#6d7d72] dark:text-[#a9b6ac]">Створи організацію у студії курсів — тоді тут можна буде запросити колег.</p><button type="button" onClick={() => navigate("/edu/courses")} className="mt-6 rounded-xl bg-[#153321] px-4 py-3 text-sm font-bold text-white dark:bg-[#00d978] dark:text-[#062211]">До курсів</button></section> : <>
        <div className="mb-7 flex gap-2 overflow-x-auto pb-1">{orgs.map((org) => <button type="button" key={org.orgId} onClick={() => void switchOrg(org)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${activeId === org.orgId ? "bg-[#17251c] text-white dark:bg-[#edf3ef] dark:text-[#102016]" : "bg-[#edf2ed] text-[#64756a] dark:bg-white/[.06] dark:text-[#adbaaf]"}`}>{org.name || "Навчальний простір"}</button>)}</div>
