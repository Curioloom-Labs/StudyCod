...
export const SupportExperience: React.FC<Props> = (props) => {
  const {
    ...
    selectedConversation, messages, newSubject, newMessage, newFiles, composerText, composerFiles,
    canCreate, canSend, fileInputRef, newFileInputRef, setNewSubject, setNewMessage, setNewFiles, setComposerText,
    setComposerFiles, selectConversation, onCreateConversation, onSendMessage, onRefresh,
    onCloseConversation, onReopenConversation, onHome, onDownloadAttachment, attachmentPreviewUrls, statusLabel, senderLabel, humanSize,
  } = props;
      ...
      <div className="min-h-[100dvh] bg-[#f7f8f5] font-sans text-[#111814] [&_h1]:font-sans [&_h2]:font-sans dark:bg-[#0b100d] dark:text-[#edf3ef]">
        ...
        <main className="mx-auto w-[min(860px,calc(100%_-_32px))] pb-40 pt-14">
          ...
          <div className="relative mt-14 before:absolute before:bottom-0 before:left-[23px] before:top-0 before:w-px before:bg-[#122017]/10 dark:before:bg-white/10">
            {threadLoading ? <div className="space-y-8 pl-16"><div className="h-24 animate-pulse rounded-2xl bg-[#e6eae4] dark:bg-[#1b231d]" /><div className="h-28 animate-pulse rounded-2xl bg-[#e6eae4] dark:bg-[#1b231d]" /></div> : messages.length ? messages.map((message, index) => {
              const fromUser = message.senderType === "USER";
              return <motion.article key={message.id} initial={reduceMotion ? undefined : { opacity: 0, y: 14 }} animate={reduceMotion ? undefined : { opacity: 1, y: 0 }} transition={{ delay: index * .06 }} className="relative pb-10 pl-16"><span className={`absolute left-2 top-0 grid size-8 place-items-center rounded-full border-[4px] border-[#f7f8f5] text-[9px] font-extrabold dark:border-[#0b100d] ${fromUser ? "bg-[#00ff88] text-[#062315]" : "bg-[#101713] text-white ring-1 ring-white/15"}`}>{fromUser ? tr("ВИ", "YOU") : "SC"}</span><div className={`${surface} rounded-[22px] p-5 shadow-[0_14px_38px_rgba(18,32,23,.045)] dark:shadow-[0_14px_38px_rgba(0,0,0,.16)]`}><div className="flex items-center justify-between gap-4"><strong className="text-[12px]">{senderLabel(message.senderType)}</strong><time className="text-[9px] text-[#7a867e] dark:text-[#849188]">{new Date(message.createdAt).toLocaleString()}</time></div>{message.text && <p className="mt-3 whitespace-pre-wrap text-[14px] leading-7 text-[#465249] dark:text-[#c2cbc5]">{message.text}</p>}{message.attachments?.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{message.attachments.map(file => <div key={file.id} className="overflow-hidden rounded-[14px] border border-[#122017]/10 bg-[#f7f8f5] dark:border-white/10 dark:bg-[#101612]">{file.mimeType.startsWith("image/") && attachmentPreviewUrls[file.id] ? <img src={attachmentPreviewUrls[file.id]} alt={file.originalName} className="max-h-64 w-full object-contain bg-black/[.03] dark:bg-white/[.03]" /> : null}<button type="button" onClick={() => onDownloadAttachment(file.id)} className="flex w-full items-center gap-3 p-3 text-left"><span className="grid size-9 place-items-center rounded-xl bg-[#00ff88]/10 text-[#00884a] dark:text-[#62ecaa]"><FileText className="size-4" /></span><span className="min-w-0 flex-1"><strong className="block truncate text-[10px]">{file.originalName}</strong><small className="mt-1 block text-[9px] text-[#7a867e]">{humanSize(file.sizeBytes)} · {tr("Скачати", "Download")}</small></span></button></div>)}</div> : null}</div></motion.article>;
            }) : <div className="pl-16 text-sm text-[#758179]">{tr("У цьому зверненні ще немає повідомлень.", "This request has no messages yet.")}</div>}
          </div>
        </main>
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[#122017]/10 bg-[#f7f8f5]/90 px-4 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-[#0b100d]/90">
          <form onSubmit={onSendMessage} className="mx-auto flex w-full max-w-[860px] items-end gap-2 rounded-[20px] border border-[#122017]/10 bg-white p-2 shadow-[0_18px_55px_rgba(18,32,23,.11)] dark:border-white/10 dark:bg-[#151c17] dark:shadow-[0_18px_55px_rgba(0,0,0,.3)]">
            <label className={`grid size-11 shrink-0 place-items-center rounded-[14px] text-[#667169] transition dark:text-[#9da9a1] ${selectedConversation?.status === "CLOSED" ? "cursor-not-allowed opacity-40" : "cursor-pointer hover:bg-[#edf0eb] dark:hover:bg-[#202821]"}`}><Paperclip className="size-4" /><input ref={fileInputRef} type="file" multiple disabled={selectedConversation?.status === "CLOSED"} className="hidden" onChange={event => setComposerFiles(Array.from(event.target.files || []))} /></label>
          ...
          {selectedConversation?.status === "OPEN" ? <button onClick={onCloseConversation} className="mx-auto mt-2 block text-[10px] font-semibold text-[#7a867e] hover:text-[#ff6b9d]">{tr("Позначити питання вирішеним", "Mark as resolved")}</button> : <button onClick={onReopenConversation} className="mx-auto mt-2 block text-[10px] font-semibold text-[#00884a] hover:text-[#006a39] dark:text-[#62ecaa]">{tr("Знову відкрити звернення", "Reopen request")}</button>}
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-[100dvh] bg-[#f7f8f5] font-sans text-[#111814] [&_h1]:font-sans [&_h2]:font-sans [&_h3]:font-sans dark:bg-[#0b100d] dark:text-[#edf3ef]">
      ...
      <main>
        ...
        <section className="mx-auto w-[min(1040px,calc(100%_-_32px))] py-20">
          ...
          <div className="mt-20 flex flex-wrap items-end justify-between gap-4"><div><span className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#00884a] dark:text-[#62ecaa]">{tr("Історія", "History")}</span><h2 className="mt-2 text-[32px] font-bold tracking-[-.045em]">{tr("Ваші звернення", "Your requests")}</h2></div><div className="flex items-center gap-2"><button type="button" onClick={onRefresh} disabled={loading} className="grid size-11 place-items-center rounded-[14px] border border-[#122017]/10 bg-white text-[#667169] transition hover:bg-[#edf0eb] disabled:opacity-50 dark:border-white/10 dark:bg-[#151c17] dark:text-[#9da9a1] dark:hover:bg-white/[.07]" aria-label={tr("Оновити звернення", "Refresh requests")}><RefreshCw className={`size-4 ${loading ? "animate-spin" : ""}`} /></button><button onClick={() => beginRequest(category)} className="flex h-11 items-center gap-2 rounded-[14px] bg-[#00ff88] px-4 text-[12px] font-bold text-[#06150d]"><Plus className="size-4" />{tr("Нове", "New")}</button></div></div>
          ...
          {error && !selectedConversationId && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#ff6b9d]/20 bg-[#ff6b9d]/10 px-4 py-3 text-[13px] text-[#c94370] dark:text-[#ff91b7]"><span>{error}</span><button type="button" onClick={onRefresh} className="font-bold underline underline-offset-2">{tr("Спробувати ще", "Try again")}</button></div>}
          <motion.section id="support-compose" initial={false} animate={{ height: composeOpen ? "auto" : 0, opacity: composeOpen ? 1 : 0, marginTop: composeOpen ? 80 : 0 }} className="overflow-hidden">
            <div className="grid grid-cols-[.72fr_1.28fr] gap-12 rounded-[28px] bg-[#101713] p-[clamp(24px,5vw,60px)] text-white max-md:grid-cols-1 max-md:gap-8"><div><span className="grid size-12 place-items-center rounded-2xl border border-white/10 bg-white/[.06] text-[#62efaa]"><MessageCircle className="size-5" /></span><h2 className="mt-7 text-[34px] font-bold leading-[1.06] tracking-[-.045em]">{tr("Дайте нам контекст", "Give us context")}</h2><p className="mt-4 text-[13px] leading-6 text-[#aab5ad]">{tr("Конкретний заголовок і кілька кроків відтворення допомагають відповісти значно швидше.", "A precise title and a few reproduction steps help us respond much faster.")}</p></div><form onSubmit={onCreateConversation} className="space-y-4"><div><label className="mb-2 block text-[11px] font-bold text-[#c2cbc5]">{tr("Коротка тема", "Short subject")}</label><input value={newSubject} onChange={event => setNewSubject(event.target.value)} className="h-12 w-full rounded-[14px] border border-white/10 bg-white/[.06] px-4 text-[13px] outline-none focus:border-[#00e97c]/45" required /></div><div><label className="mb-2 block text-[11px] font-bold text-[#c2cbc5]">{tr("Що відбулося?", "What happened?")}</label><textarea value={newMessage} onChange={event => setNewMessage(event.target.value)} className="min-h-[150px] w-full resize-y rounded-[14px] border border-white/10 bg-white/[.06] p-4 text-[13px] leading-6 outline-none focus:border-[#00e97c]/45" required /></div><div className="flex flex-wrap items-center gap-3"><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/[.05] px-3 text-[11px] font-semibold text-[#c2cbc5] transition hover:bg-white/[.1]"><Paperclip className="size-3.5" />{tr("Додати скріншот або файл", "Add a screenshot or file")}<input ref={newFileInputRef} type="file" multiple className="hidden" onChange={event => setNewFiles(Array.from(event.target.files || []))} /></label>{newFiles.length > 0 && <span className="text-[10px] text-[#aab5ad]">{newFiles.length} {tr("файл(и) готові", "file(s) ready")}</span>}</div>{error && <p className="text-[11px] text-[#ff8fb6]">{error}</p>}<div className="flex justify-end"><button type="submit" disabled={!canCreate || sending} className="flex h-12 items-center gap-2 rounded-[14px] bg-[#00ff88] px-5 text-[12px] font-bold text-[#06150d] disabled:opacity-50">{sending ? tr("Створюємо…", "Creating…") : tr("Надіслати звернення", "Send request")}<ArrowRight className="size-4" /></button></div></form></div>
          </motion.section>
        </section>
      </main>
    </div>
