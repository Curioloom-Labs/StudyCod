...
export const SupportExperience: React.FC<Props> = (props) => {
  ...
  const renderAttachment = (file: SupportChatMessage["attachments"][number]) => {
    ...
    return <div key={file.id} className="overflow-hidden rounded-2xl border border-white/[.09] bg-[#0b120e]">
      {preview ? <button type="button" onClick={() => setLightbox({ url: preview, name: file.originalName })} className="group block w-full bg-black/20" aria-label={`${tr("Відкрити", "Open")} ${file.originalName}`}><img src={preview} alt={file.originalName} width={1200} height={800} loading="lazy" className="max-h-72 w-full object-contain transition duration-300 group-hover:scale-[1.02]" /></button> : <div className="grid h-28 place-items-center bg-[#0b120e] text-[#7e9183]"><FileText className="size-7" /></div>}
  ...
  if (selectedConversationId) {
    ...
    return <div className="min-h-[100dvh] bg-[#07100a] font-sans text-[#edf5ef]">
      ...
      <main id="main-content" className="mx-auto max-w-[1320px] px-4 pb-10 pt-5 sm:px-6 lg:px-8 lg:pt-8">
