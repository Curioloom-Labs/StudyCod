...
export const TheoryPage: React.FC<TheoryPageProps> = ({
  ...
  return <div className="flex flex-col md:flex-row gap-4 md:gap-6 p-3 sm:p-4 md:p-6">
      <div className="md:w-1/3 bg-bg-surface rounded-xl border border-border overflow-hidden flex flex-col">
        ...
        <div className="flex-1 p-2 space-y-1">
          ...
          {!loading && loadError && <div className="p-3 text-sm font-mono text-accent-error border border-accent-error rounded-lg bg-bg-code">{loadError}</div>}
          ...
          {!loading && topics.length === 0 && !loadError && <div className="p-3 text-sm font-mono text-text-secondary">{tr("Немає теорії для цього курсу", "No theory available for this course")}</div>}
        </div>
      </div>

      <div className="md:w-2/3 bg-bg-surface rounded-xl border border-border p-4 sm:p-6">
        {selected ? <div>
