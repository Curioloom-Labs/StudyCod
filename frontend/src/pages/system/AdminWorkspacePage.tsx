...
export const AdminWorkspacePage: React.FC = () => {
  ...
  const [newUser, setNewUser] = useState({ username: "", email: "", password: "", firstName: "", lastName: "", role: "USER" as AdminUser["role"], userMode: "PERSONAL" as AdminUser["userMode"] });
  ...
  const createUser = async (event: React.FormEvent) => {
    ...
    try { await createAdminUser(newUser); setShowCreateUser(false); setNewUser({ username: "", email: "", password: "", firstName: "", lastName: "", role: "USER", userMode: "PERSONAL" }); await loadPeople(1, false); } catch (cause) { reportError(cause, "Не вдалося створити користувача."); }
  };
