...
type UserDraft = { email: string; firstName: string; lastName: string; password: string; role: AdminUser["role"] };
type NewUserDraft = { username: string; email: string; password: string; firstName: string; lastName: string; role: AdminUser["role"]; userMode: AdminUser["userMode"] };
type BroadcastDraft = { subject: string; title: string; content: string; delivery: "MARKETING" | "NOTIFICATION"; emails: string; classIds: string; userIds: string; includeSubscribed: boolean; includeAllUsers: boolean; confirm: string; dryRun: boolean };
...
export const AdminWorkspacePage: React.FC = () => {
  ...
  const [classDraft, setClassDraft] = useState({ id: 0, name: "", teacherId: 0 });
  ...
  const saveClass = async () => {
    try {
      if (classDraft.id) await updateAdminClass(classDraft.id, { name: classDraft.name, teacherId: Number(classDraft.teacherId) });
      else await createAdminClass({ name: classDraft.name, teacherId: Number(classDraft.teacherId) });
      setClassDraft({ id: 0, name: "", teacherId: 0 }); const r = await getAdminClasses(); setClasses(r.classes);
...
const ClassesPanel: React.FC<{ classes: AdminClass[]; users: AdminUser[]; draft: { id: number; name: string; teacherId: number }; setDraft: React.Dispatch<React.SetStateAction<{ id: number; name: string; teacherId: number }>>; onSave: () => void; onDelete: (item: AdminClass) => void }> = ({ classes, users, draft, setDraft, onSave, onDelete }) => { const navigate = useNavigate(); return <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]"><Surface><Label>Клас</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">{draft.id ? "Редагувати клас" : "Новий клас"}</h2><div className="mt-5 space-y-3"><Field label="Назва" value={draft.name} onChange={(e) => setDraft((p) => ({ ...p, name: e.target.value }))} /><Select label="Викладач" value={draft.teacherId} onChange={(e) => setDraft((p) => ({ ...p, teacherId: Number(e.target.value) }))}><option value={0}>Оберіть викладача</option>{users.filter((u) => u.role === "TEACHER" || u.role === "SYSTEM_ADMIN").map((u) => <option key={u.id} value={u.id}>{u.username}</option>)}</Select><div className="flex gap-2"><PrimaryButton onClick={onSave}><Save className="mr-2 inline h-4 w-4" />Зберегти</PrimaryButton><PrimaryButton muted onClick={() => setDraft({ id: 0, name: "", teacherId: 0 })}>Очистити</PrimaryButton></div></div></Surface><Surface><Label>Усі класи</Label><div className="mt-5 space-y-2">{classes.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-[#f5f8f5] p-4 dark:bg-white/[.04]"><div><div className="font-semibold">{item.name}</div><div className="text-sm text-[#708075] dark:text-[#a4b2a7]">{item.teacherName}</div></div><div className="flex gap-2"><PrimaryButton muted onClick={() => navigate(`/edu/classes/${item.id}`)}>Відкрити</PrimaryButton><PrimaryButton muted onClick={() => setDraft({ id: item.id, name: item.name, teacherId: item.teacherId })}>Редагувати</PrimaryButton><PrimaryButton muted onClick={() => onDelete(item)}><Trash2 className="h-4 w-4" /></PrimaryButton></div></div>)}</div></Surface></div>; };
...
const BroadcastPanel: React.FC<{ broadcast: BroadcastDraft; setBroadcast: React.Dispatch<React.SetStateAction<BroadcastDraft>>; result: unknown; send: (event: React.FormEvent) => void }> = ({ broadcast, setBroadcast, result, send }) => <Surface><Label>Розсилки</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">Email/notification broadcast</h2><form onSubmit={send} className="mt-5 grid gap-4 lg:grid-cols-[1fr_360px]"><div className="space-y-3"><Field label="Subject" value={broadcast.subject} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, subject: e.target.value }))} /><Field label="Title" value={broadcast.title} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, title: e.target.value }))} /><TextArea label="Content" rows={8} value={broadcast.content} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, content: e.target.value }))} /><div className="flex flex-wrap gap-2"><PrimaryButton type="submit" onClick={() => setBroadcast((p: BroadcastDraft) => ({ ...p, dryRun: true }))}>Dry run</PrimaryButton><PrimaryButton muted type="submit" onClick={() => setBroadcast((p: BroadcastDraft) => ({ ...p, dryRun: false }))}>Send</PrimaryButton></div></div><div className="space-y-3"><Select label="Delivery" value={broadcast.delivery} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, delivery: e.target.value as BroadcastDraft["delivery"] }))}><option value="NOTIFICATION">Notification</option><option value="MARKETING">Marketing</option></Select><TextArea label="Emails" rows={3} value={broadcast.emails} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, emails: e.target.value }))} /><TextArea label="Class IDs" rows={2} value={broadcast.classIds} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, classIds: e.target.value }))} /><TextArea label="User IDs" rows={2} value={broadcast.userIds} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, userIds: e.target.value }))} /><label className="flex gap-2 text-sm"><input type="checkbox" checked={broadcast.includeSubscribed} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, includeSubscribed: e.target.checked }))} />Include subscribed</label><label className="flex gap-2 text-sm"><input type="checkbox" checked={broadcast.includeAllUsers} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, includeAllUsers: e.target.checked }))} />Notify all users</label><Field label="Confirm" value={broadcast.confirm} onChange={(e) => setBroadcast((p: BroadcastDraft) => ({ ...p, confirm: e.target.value }))} /></div></form>{result ? <pre className="mt-4 overflow-auto rounded-2xl bg-[#f5f8f5] p-4 text-xs dark:bg-white/[.04]">{JSON.stringify(result, null, 2)}</pre> : null}</Surface>;
...
const CreateUserModal: React.FC<{ newUser: NewUserDraft; setNewUser: React.Dispatch<React.SetStateAction<NewUserDraft>>; close: () => void; submit: (e: React.FormEvent) => void }> = ({ newUser, setNewUser, close, submit }) => <ModalFrame close={close}><form onSubmit={submit} className="space-y-3"><Label>Створення акаунта</Label><p className="text-sm leading-6 text-[#708075] dark:text-[#a4b2a7]">Для викладача обери роль TEACHER: режим EDU встановиться автоматично.</p><div className="grid gap-3 sm:grid-cols-2"><Field label="Ім’я" value={newUser.firstName} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, firstName: e.target.value }))} /><Field label="Прізвище" value={newUser.lastName} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, lastName: e.target.value }))} /></div><Field label="Username" required value={newUser.username} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, username: e.target.value }))} /><Field label="Email" value={newUser.email} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, email: e.target.value }))} /><Field label="Password" required type="password" value={newUser.password} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, password: e.target.value }))} /><div className="grid grid-cols-2 gap-3"><Select label="Role" value={newUser.role} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, role: e.target.value as NewUserDraft["role"] }))}><option>USER</option><option>TEACHER</option><option>SUPPORT</option><option>SYSTEM_ADMIN</option></Select><Select label="Mode" value={newUser.userMode} onChange={(e) => setNewUser((p: NewUserDraft) => ({ ...p, userMode: e.target.value as NewUserDraft["userMode"] }))}><option>PERSONAL</option><option>EDUCATIONAL</option><option>CONTEST</option></Select></div><PrimaryButton className="w-full">Створити акаунт і показати дані</PrimaryButton></form></ModalFrame>;
...
const UserModal: React.FC<{ selectedUser: AdminUser; draft: UserDraft; setDraft: React.Dispatch<React.SetStateAction<UserDraft>>; close: () => void; save: () => void; remove: () => void }> = ({ selectedUser, draft, setDraft, close, save, remove }) => <ModalFrame close={close}><Label>Користувач</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">@{selectedUser.username}</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><Field label="Email" value={draft.email} onChange={(e) => setDraft((p: UserDraft) => ({ ...p, email: e.target.value }))} /><Field label="First name" value={draft.firstName} onChange={(e) => setDraft((p: UserDraft) => ({ ...p, firstName: e.target.value }))} /><Field label="Last name" value={draft.lastName} onChange={(e) => setDraft((p: UserDraft) => ({ ...p, lastName: e.target.value }))} /><Field label="New password" type="password" value={draft.password} onChange={(e) => setDraft((p: UserDraft) => ({ ...p, password: e.target.value }))} /><Select label="Role" value={draft.role} onChange={(e) => setDraft((p: UserDraft) => ({ ...p, role: e.target.value as UserDraft["role"] }))}><option>USER</option><option>TEACHER</option><option>SUPPORT</option><option>SYSTEM_ADMIN</option></Select></div><div className="mt-5 flex flex-wrap gap-2"><PrimaryButton onClick={save}>Зберегти</PrimaryButton><PrimaryButton muted onClick={remove}><Trash2 className="mr-2 inline h-4 w-4" />Видалити</PrimaryButton></div></ModalFrame>;
const CertificateModal: React.FC<{ details: CertificateTemplate; draft: CertificateDraft; setDraft: React.Dispatch<React.SetStateAction<CertificateDraft>>; close: () => void; save: () => void }> = ({ details, draft, setDraft, close, save }) => {
  ...
  const addField = (fieldKey: string) => setDraft((p: CertificateDraft) => ({ ...p, htmlTemplate: `${String(p.htmlTemplate || "").trim()}\n<span>{{${fieldKey}}}</span>` }));
  return (
    <ModalFrame close={close} wide>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-[1fr_180px_140px]">
        <Field label="Назва" value={draft.name} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, name: e.target.value }))} />
        <Select label="Тип" value={draft.type} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, type: e.target.value as CertificateDraft["type"] }))}>
          <option value="custom">custom</option>
          <option value="studycod">studycod</option>
        </Select>
        <label className="mt-8 flex items-center gap-2 text-sm">
          <input type="checkbox" checked={draft.isActive} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, isActive: e.target.checked }))} />
          Активний
        </label>
      </div>
      <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_1fr]">
        <div className="space-y-3">
          <TextArea label="HTML" rows={12} value={draft.htmlTemplate} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, htmlTemplate: e.target.value }))} className="font-mono text-xs" />
          <TextArea label="CSS" rows={12} value={draft.cssTemplate} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, cssTemplate: e.target.value }))} className="font-mono text-xs" />
          <div>
          ...
          return (
            <div key={fieldKey} className="rounded-xl bg-[#f5f8f5] p-3 text-sm dark:bg-white/[.04]">
              <div className="flex items-center justify-between gap-2">
                ...
                <input id={enabledId} type="checkbox" aria-label={`Enable ${fieldKey}`} checked={current.isEnabled} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, fields: upsertField(p.fields, fieldKey, { isEnabled: e.target.checked, isRequired: e.target.checked ? current.isRequired : false }) }))} />
              </div>
              <label htmlFor={requiredId} className="mt-2 flex gap-2 text-xs text-[#708075]">
                <input id={requiredId} type="checkbox" disabled={!current.isEnabled} checked={current.isRequired} onChange={(e) => setDraft((p: CertificateDraft) => ({ ...p, fields: upsertField(p.fields, fieldKey, { isEnabled: current.isEnabled, isRequired: e.target.checked }) }))} />
                обов’язкове
              </label>
            </div>
          );
...
function upsertField(fields: CertificateTemplate["fields"], fieldKey: typeof fieldKeys[number], patch: { isEnabled: boolean; isRequired: boolean }): CertificateTemplate["fields"] {
  ...
  const order = new Map<string, number>(fieldKeys.map((key, index) => [key, index]));
  return next.sort((a, b) => (order.get(a.fieldKey) ?? fieldKeys.length) - (order.get(b.fieldKey) ?? fieldKeys.length));
}
