...
import { getCertificateTemplateById, listCertificateTemplates, updateCertificateTemplate } from "../../lib/api/certificates";
...
export const AdminWorkspacePage: React.FC = () => {
  ...
  const [userDraft, setUserDraft] = useState({ email: "", firstName: "", lastName: "", password: "", role: "USER" as AdminUser["role"], lang: "PYTHON" as AdminUser["lang"] });
  const [showCreateUser, setShowCreateUser] = useState(false);
  const [newUser, setNewUser] = useState({ username: "", email: "", password: "", firstName: "", lastName: "", role: "USER" as AdminUser["role"], userMode: "PERSONAL" as AdminUser["userMode"], lang: "PYTHON" as AdminUser["lang"] });
  ...
  const load = async () => {
    ...
    if (isPreview()) {
      const demoUsers = [{ id: 1, username: "oksana", firstName: "Оксана", lastName: "Коваль", email: "oksana@example.test", role: "USER", userMode: "PERSONAL", lang: "PYTHON", emailVerified: true, iad: 0, avatarUrl: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }] as AdminUser[];
    ...
    try {
      ...
      if (tab === "support") { const [chat, legacy] = await Promise.all([getAdminSupportConversations(), getAdminSupportTickets().catch(() => ({ tickets: [] }))]); setConversations(chat.conversations); setSupportTickets(legacy.tickets); }
      ...
      if (tab === "maintenance") { const r = await getAdminMaintenance(); setMaintenance(r.state); setMaintenanceDraft({ title: r.state.title || "Технічне обслуговування", message: r.state.message || "", until: r.state.until || "" }); }
  ...
  const selectUser = (user: AdminUser) => {
    ...
    setUserDraft({ email: user.email || "", firstName: user.firstName || "", lastName: user.lastName || "", password: "", role: user.role, lang: user.lang });
  };
  const saveSelectedUser = async () => {
    if (!selectedUser) return;
    try {
      await updateAdminUser(selectedUser.id, { email: userDraft.email || undefined, firstName: userDraft.firstName, lastName: userDraft.lastName, password: userDraft.password || undefined, lang: userDraft.lang });
  ...
  const createUser = async (event: React.FormEvent) => {
    ...
    try { await createAdminUser(newUser); setShowCreateUser(false); setNewUser({ username: "", email: "", password: "", firstName: "", lastName: "", role: "USER", userMode: "PERSONAL", lang: "PYTHON" }); await loadPeople(1, false); } catch (cause) { reportError(cause, "Не вдалося створити користувача."); }
  };
  ...
  const replySupport = async () => {
    ...
    try { await postAdminSupportConversationMessage(selectedConversationId, { text: supportReply, sendEmail: true }); setSupportReply(""); await openConversation(selectedConversationId); } catch (cause) { reportError(cause, "Не вдалося відповісти."); }
  };
  ...
  const saveMaintenance = async (enabled: boolean) => {
    try {
      const next = enabled ? await enableAdminMaintenance({ title: maintenanceDraft.title, message: maintenanceDraft.message, until: maintenanceDraft.until || null }) : await disableAdminMaintenance();
  ...
  const saveCertificate = async () => {
    ...
    try {
      await updateCertificateTemplate(certificateDetails.id, certificateDraft);
  ...
  return <div className="min-h-full bg-[#f7f8f5] px-4 py-7 text-[#142017] dark:bg-[#0b120e] dark:text-[#edf3ef] sm:px-6 lg:px-10 lg:py-10"><div className="mx-auto max-w-[1480px]">
    ...
    {loading ? <div className="mt-5 grid gap-4 md:grid-cols-3">{Array.from({ length: 6 }, (_, index) => <div key={index} className="h-40 animate-pulse rounded-[24px] bg-[#e6ece7] dark:bg-white/[.045]" />)}</div> : <div className="mt-5">
      ...
      {tab === "certificates" && <CertificatesPanel certificates={certificates} open={openCertificate} />}
    </div>}
...
const SupportPanel: React.FC<{ conversations: AdminSupportChatConversation[]; selectedConversationId: number | null; messages: AdminSupportChatMessage[]; reply: string; setReply: (v: string) => void; openConversation: (id: number) => void; replySupport: () => void; tickets: AdminSupportTicket[]; selectedTicket: AdminSupportTicket | null; setSelectedTicket: (ticket: AdminSupportTicket | null) => void; ticketReply: string; setTicketReply: (value: string) => void; replyTicket: () => void }> = ({ conversations, selectedConversationId, messages, reply, setReply, openConversation, replySupport, tickets, selectedTicket, setSelectedTicket, ticketReply, setTicketReply, replyTicket }) => <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]"><div className="space-y-5"><Surface><Label>Підтримка</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">Відкриті діалоги</h2><div className="mt-5 space-y-2">{conversations.map((item) => <button key={item.id} onClick={() => openConversation(item.id)} className={`w-full rounded-2xl p-4 text-left ${selectedConversationId === item.id ? "bg-[#e8f6ed] dark:bg-[#00ff88]/10" : "bg-[#f5f8f5] dark:bg-white/[.04]"}`}><div className="font-semibold">{item.subject}</div><div className="mt-1 text-sm text-[#708075] dark:text-[#a4b2a7]">{item.userEmail} · {item.status}</div></button>)}{!conversations.length && <div className="text-sm text-[#708075]">Нових чатів немає.</div>}</div></Surface><Surface><Label tone="text-[#d97706]">Legacy tickets</Label><p className="mt-2 text-sm text-[#708075] dark:text-[#a4b2a7]">Старі звернення з email-відповіддю перенесені сюди, без повернення в старий UI.</p><div className="mt-4 space-y-2">{tickets.map((ticket) => <button key={ticket.id} onClick={() => setSelectedTicket(ticket)} className={`w-full rounded-2xl p-4 text-left ${selectedTicket?.id === ticket.id ? "bg-[#fff8ec] dark:bg-[#ff8c00]/10" : "bg-[#f5f8f5] dark:bg-white/[.04]"}`}><div className="flex items-center justify-between gap-3"><span className="font-semibold">{ticket.subject}</span><Pill>{ticket.status}</Pill></div><div className="mt-1 text-sm text-[#708075] dark:text-[#a4b2a7]">{ticket.userEmail}</div></button>)}{!tickets.length && <div className="text-sm text-[#708075]">Legacy tickets порожні.</div>}</div></Surface></div><div className="space-y-5"><Surface><Label>Чат</Label><div className="mt-5 max-h-[360px] overflow-auto space-y-3">{messages.map((m) => <div key={m.id} className={`rounded-2xl p-3 ${m.senderType === "ADMIN" ? "bg-[#e8f6ed] dark:bg-[#00ff88]/10" : "bg-[#f5f8f5] dark:bg-white/[.04]"}`}><div className="text-xs font-semibold uppercase tracking-[.12em] text-[#708075]">{m.senderType} · {new Date(m.createdAt).toLocaleString()}</div><div className="mt-2 whitespace-pre-wrap text-sm">{m.text}</div></div>)}{!messages.length && <div className="text-sm text-[#708075]">Оберіть діалог.</div>}</div><TextArea rows={4} value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Відповідь..." /><PrimaryButton className="mt-3" onClick={replySupport}>Надіслати відповідь</PrimaryButton></Surface><Surface><Label>Legacy ticket</Label>{selectedTicket ? <div className="mt-4 space-y-4"><div className="rounded-2xl bg-[#f5f8f5] p-4 text-sm dark:bg-white/[.04]"><div className="font-semibold">{selectedTicket.subject}</div><div className="mt-1 text-[#708075] dark:text-[#a4b2a7]">{selectedTicket.userEmail} · {selectedTicket.status}</div><p className="mt-3 whitespace-pre-wrap">{selectedTicket.message}</p></div><TextArea rows={5} value={ticketReply} onChange={(e) => setTicketReply(e.target.value)} placeholder="Email-відповідь користувачу..." /><PrimaryButton onClick={replyTicket}>Відповісти на ticket</PrimaryButton></div> : <div className="mt-4 text-sm text-[#708075]">Оберіть legacy ticket.</div>}</Surface></div></div>;
...
const CertificatesPanel: React.FC<{ certificates: CertificateListItem[]; open: (id: number) => void }> = ({ certificates, open }) => <Surface><Label>Сертифікати</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">Шаблони сертифікатів</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#708075] dark:text-[#a4b2a7]">Тепер редагування HTML/CSS/полів перенесено сюди: відкрий шаблон і збережи зміни без переходу у стару адмінку.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{certificates.map((item) => <button type="button" onClick={() => open(item.id)} key={item.id} className="rounded-2xl border border-[#152219]/8 bg-[#fafcf9] p-4 text-left transition hover:-translate-y-0.5 hover:border-[#00c96d]/35 dark:border-white/8 dark:bg-white/[.025]"><div className="font-semibold">{item.name}</div><div className="mt-2 text-sm text-[#728176] dark:text-[#a4b2a7]">{item.isActive ? "Активний шаблон" : "Чернетка або вимкнений"}</div><div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-[#64736a] dark:text-[#a5b4a8]"><Pill>{item.type || "template"}</Pill>{item.version ? <Pill>v{item.version}</Pill> : null}{item.contestId ? <Pill>contest #{item.contestId}</Pill> : null}</div></button>)}</div></Surface>;
...
const CertificateModal: React.FC<{ details: CertificateTemplate; draft: { name: string; type: "studycod" | "custom"; htmlTemplate: string; cssTemplate: string; isActive: boolean; fields: CertificateTemplate["fields"] }; setDraft: React.Dispatch<React.SetStateAction<any>>; close: () => void; save: () => void }> = ({ details, draft, setDraft, close, save }) => <ModalFrame close={close} wide><Label>Сертифікат #{details.id}</Label><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">{draft.name}</h2><div className="mt-5 grid gap-3 md:grid-cols-[1fr_180px_140px]"><Field label="Name" value={draft.name} onChange={(e) => setDraft((p: any) => ({ ...p, name: e.target.value }))} /><Select label="Type" value={draft.type} onChange={(e) => setDraft((p: any) => ({ ...p, type: e.target.value }))}><option value="custom">custom</option><option value="studycod">studycod</option></Select><label className="mt-8 flex items-center gap-2 text-sm"><input type="checkbox" checked={draft.isActive} onChange={(e) => setDraft((p: any) => ({ ...p, isActive: e.target.checked }))} />Active</label></div><div className="mt-4 grid gap-3 lg:grid-cols-2"><TextArea label="HTML" rows={14} value={draft.htmlTemplate} onChange={(e) => setDraft((p: any) => ({ ...p, htmlTemplate: e.target.value }))} className="font-mono text-xs" /><TextArea label="CSS" rows={14} value={draft.cssTemplate} onChange={(e) => setDraft((p: any) => ({ ...p, cssTemplate: e.target.value }))} className="font-mono text-xs" /></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{fieldKeys.map((fieldKey) => { const current = draft.fields.find((f) => f.fieldKey === fieldKey) ?? { fieldKey, isEnabled: false, isRequired: false }; return <label key={fieldKey} className="rounded-xl bg-[#f5f8f5] p-3 text-sm dark:bg-white/[.04]"><div className="flex items-center justify-between gap-2"><span className="font-semibold">{fieldKey}</span><input type="checkbox" checked={current.isEnabled} onChange={(e) => setDraft((p: any) => ({ ...p, fields: upsertField(p.fields, fieldKey, { isEnabled: e.target.checked, isRequired: e.target.checked ? current.isRequired : false }) }))} /></div><label className="mt-2 flex gap-2 text-xs text-[#708075]"><input type="checkbox" disabled={!current.isEnabled} checked={current.isRequired} onChange={(e) => setDraft((p: any) => ({ ...p, fields: upsertField(p.fields, fieldKey, { isEnabled: current.isEnabled, isRequired: e.target.checked }) }))} />required</label></label>; })}</div><div className="mt-5 flex justify-end"><PrimaryButton onClick={save}>Оновити шаблон</PrimaryButton></div></ModalFrame>;

const ModalFrame: React.FC<{ children: React.ReactNode; close: () => void; wide?: boolean }> = ({ children, close, wide }) => <div className="fixed inset-0 z-[70] grid place-items-center bg-[#07110b]/55 p-4 backdrop-blur-sm"><section className={`max-h-[88dvh] w-full overflow-auto rounded-[26px] bg-white p-6 text-[#142017] shadow-2xl dark:bg-[#121b15] dark:text-[#edf3ef] ${wide ? "max-w-6xl" : "max-w-xl"}`}><button type="button" onClick={close} className="float-right rounded-xl p-2 transition hover:bg-[#f1f5f1] dark:hover:bg-white/[.06]"><X className="h-5 w-5" /></button>{children}</section></div>;
