...
export const GoogleAuthCompletePage: React.FC<Props> = ({ onAuth }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!setupToken) return setError(tr("Сесія Google завершилася. Спробуйте увійти ще раз.", "The Google session has expired. Please sign in again."));
  ...
  if (!setupToken) {
    return <div className="grid min-h-[100dvh] place-items-center bg-[#f7f8f5] px-5 text-[#111814] dark:bg-[#0c110e] dark:text-white"><div className="w-full max-w-[440px] space-y-5"><span className="grid size-12 place-items-center rounded-2xl bg-[#00ff88] text-[#07140d]"><AlertCircle className="size-5" /></span><h1 className="text-3xl font-bold tracking-[-.05em]">{tr("Не вдалося продовжити", "Unable to continue")}</h1><p className="text-sm leading-6 text-[#667169] dark:text-[#9faba3]">{error}</p><button type="button" onClick={() => navigate("/", { replace: true })} className={primaryButtonClass}>{tr("Повернутися на головну", "Back to home")}<ArrowRight className="size-4" /></button></div></div>;
  }
      ...
      <div className="grid min-h-[100dvh] grid-cols-[minmax(380px,.92fr)_minmax(560px,1.08fr)] max-[980px]:grid-cols-1">
        ...
        <main className="relative flex min-h-[100dvh] items-center justify-center px-6 py-10 max-sm:px-4">
          ...
          <motion.div initial={prefersReducedMotion ? undefined : { opacity: 0, y: 18 }} animate={prefersReducedMotion ? undefined : { opacity: 1, y: 0 }} transition={{ duration: .55, ease: [0.16, 1, .3, 1] }} className="w-full max-w-[560px] py-16">
            <button type="button" onClick={() => navigate("/", { replace: true })} className="mb-9 inline-flex items-center gap-2 text-[13px] font-semibold text-[#667169] transition hover:text-[#111814] dark:text-[#94a198] dark:hover:text-white"><ArrowLeft className="size-4" />{tr("На головну", "Back to home")}</button>
