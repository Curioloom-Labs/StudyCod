...
export const ContestProblemSolvePage: React.FC = () => {
  ...
  const loadSubmissions = React.useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      const rows = Array.isArray((res as any)?.submissions) ? ((res as any).submissions as ContestSubmissionListItem[]) : [];
      setSubmissions(rows);
  ...
  const hydrateDraft = React.useCallback(
    (stmt: ContestProblemStatement) => {
      ...
      try {
        ...
        const savedNotes = localStorage.getItem(`${storageBase}:notes`);
        setNotes(savedNotes ?? "");
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem(`${storageBase}:notes`, notes);
      localStorage.setItem(`${storageBase}:focus`, focusMode ? "1" : "0");
    } catch {
      // ignore
    }
  }, [statement, storageBase, judgeLanguage, code, runInput, notes, focusMode]);
  ...
  React.useEffect(() => {
    ...
    const openSocket = () => {
      try {
        ...
        ws.onmessage = (event) => {
          if (closed) return;
          const payload = safeJsonParse<any>(String(event.data ?? ""));
  ...
  }, [contestId, problemId, hasToken, token, syncLiveData]);
  ...
  const doSubmit = async () => {
    ...
    if (shouldEnforceTurnstileClient && !turnstileToken) {
      setError("Please complete human verification before submit.");
      return;
    }
