...
export const PremiumProgress: React.FC<{
  ...
  return <div className="min-h-full bg-[#f7f8f5] px-4 py-6 text-[#142017] dark:bg-[#0b120e] dark:text-[#edf3ef] sm:px-6 lg:px-10 lg:py-9"><div className="mx-auto max-w-7xl space-y-6 lg:space-y-8">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{metricCards.map(({ label, value, Icon, color }) => <div key={label} className="rounded-2xl border border-[#152219]/10 bg-white p-5 dark:border-white/10 dark:bg-[#121b15]"><div className="flex items-center justify-between text-sm text-[#6c7a70] dark:text-[#9cab9f]"><span>{label}</span><Icon className="h-4 w-4" style={{ color }} /></div><div className="mt-4 text-3xl font-semibold tracking-[-.045em]">{value}</div></div>)}</div>
...
export const PremiumProfileV2: React.FC<{
  ...
  const badges = [
    ...
    { threshold: 15, name: label("Стабільний ритм", "Steady rhythm"), detail: label("Закрий 15 задач", "Complete 15 problems") },
    { threshold: 30, name: label("Серйозна практика", "Real practice"), detail: label("Закрий 30 задач", "Complete 30 problems") },
    { threshold: 60, name: label("Системне мислення", "Systems thinker"), detail: label("Закрий 60 задач", "Complete 60 problems") },
  ];
