...
export const PremiumDashboard: React.FC<{
  ...
  const active = actionableTasks.find((task) => task.status === "OPEN")
    ?? actionableTasks.find((task) => task.status === "SUBMITTED")
    ?? actionableTasks[0];
  ...
  const shortQueue = actionableTasks.filter((task) => task.id !== active?.id).slice(0, 3);
  const activeIsWaiting = active?.status === "SUBMITTED";
      ...
      <div className="mx-auto max-w-7xl space-y-6 lg:space-y-8">
          ...
          <div className="relative grid gap-7 lg:grid-cols-[1fr_320px] lg:items-end">
            <div>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button type="button" onClick={() => active && !activeIsWaiting ? onOpenTask(active) : onNavigate(activeIsWaiting ? "grades" : "tasks")} className="inline-flex items-center gap-2 rounded-xl bg-[#142017] px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_28px_-14px_rgba(0,0,0,.55)] transition hover:-translate-y-0.5 hover:bg-[#223129] dark:bg-[#edf3ef] dark:text-[#0b120e]">
                  <Play className="h-4 w-4 fill-current" /> {activeIsWaiting ? (en ? "View review" : "Переглянути перевірку") : c.continue}
                </button>
                <button type="button" onClick={() => window.location.assign(import.meta.env.DEV ? "/library?preview=true" : "/library")} className="inline-flex items-center gap-2 rounded-xl border border-[#152219]/12 bg-white/70 px-5 py-3 text-sm font-semibold text-[#25342b] transition hover:bg-white dark:border-white/10 dark:bg-white/5 dark:text-[#e8f0ea] dark:hover:bg-white/10">
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(290px,.7fr)]">
          ...
          <section className="rounded-[26px] border border-[#152219]/10 bg-[#fbfcfa] p-5 dark:border-white/10 dark:bg-[#101813] sm:p-6" aria-labelledby="personal-path-title">
            <div className="flex items-center justify-between"><div><div className="text-xs font-semibold uppercase tracking-[.16em] text-[#e87d00]">{c.smartPath}</div><h2 id="personal-path-title" className="mt-2 text-xl font-semibold tracking-tight">{c.progress}</h2></div><Gauge className="h-5 w-5 text-[#e87d00]" /></div>
            <p className="mt-4 text-sm leading-6 text-[#617066] dark:text-[#a2b0a5]">{c.pathCopy}</p>
            <button type="button" onClick={() => onNavigate("grades")} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#147b47] transition hover:text-[#00a65a] dark:text-[#62ecaa]">{c.explore}<ArrowRight className="h-4 w-4" /></button>
        ...
        <section className="rounded-[26px] border border-[#152219]/10 bg-white p-5 dark:border-white/10 dark:bg-[#121b15] sm:p-6">
          <div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-semibold tracking-tight">{c.queue}</h2><button type="button" onClick={() => onNavigate("tasks")} className="text-sm font-semibold text-[#147b47] dark:text-[#62ecaa]">{c.allTasks}</button></div>
...
export const PremiumProfileV2: React.FC<{
...
}> = ({ user, avatarUrl, course, stats, currentIad, weeklyActiveDays, skillEvidence, saving, message, onAvatar, onSave, isEducational = false, className, canEditProfile = true }) => {
  const { i18n } = useTranslation();
  ...
  const scopeLabel = isEducational ? label("EDU акаунт", "EDU account") : label("Особистий акаунт", "Personal account");
  const profileTitle = isEducational ? label("Профіль у класі", "Class profile") : label("Твій профіль", "Your profile");
  const profileCopy = isEducational
    ? label("Твій навчальний контекст, прогрес і налаштування зібрані в одному спокійному просторі.", "Your class context, progress, and settings live together in one calm space.")
    : label("Тут зберігається твій навчальний контекст — без зайвого шуму.", "Your learning context lives here, without unnecessary noise.");
  const roleLabel = user.studentId ? label("Учень", "Student") : label("Викладач", "Teacher");
  const contextLabel = isEducational ? className || roleLabel : courseName;
  ...
  return <div className="min-h-full bg-[#f7f8f5] px-4 py-8 text-[#142017] dark:bg-[#0b120e] dark:text-[#edf3ef] sm:px-6 lg:px-10 lg:py-12"><div className="mx-auto max-w-6xl">
    <div className="mb-8 flex flex-col justify-between gap-4 border-b border-[#152219]/10 pb-7 dark:border-white/10 sm:flex-row sm:items-end"><div><div className="flex items-center gap-2 text-sm font-semibold text-[#147b47] dark:text-[#62ecaa]"><ShieldCheck className="h-4 w-4" />{scopeLabel}</div><h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-bold tracking-[-.055em] sm:text-5xl">{profileTitle}</h1><p className="mt-3 max-w-xl text-base leading-7 text-[#65746a] dark:text-[#a4b3a7]">{profileCopy}</p></div><div className="flex flex-wrap gap-2">{!isEducational && <><button type="button" onClick={() => window.location.assign("/iad")} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#152219]/10 bg-white px-4 py-3 text-sm font-semibold text-[#314139] transition hover:-translate-y-0.5 hover:bg-[#eef4ef] dark:border-white/10 dark:bg-white/[.05] dark:text-[#dce7df] dark:hover:bg-white/[.08]"><Gauge className="h-4 w-4" />IAD</button><button type="button" onClick={() => window.location.assign("/profile/certificates")} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#152219]/10 bg-white px-4 py-3 text-sm font-semibold text-[#314139] transition hover:-translate-y-0.5 hover:bg-[#eef4ef] dark:border-white/10 dark:bg-white/[.05] dark:text-[#dce7df] dark:hover:bg-white/[.08]"><Award className="h-4 w-4" />{label("Сертифікати", "Certificates")}</button></>}{canEditProfile && <button type="button" onClick={onSave} disabled={saving} className="inline-flex shrink-0 items-center justify-center rounded-xl bg-[#00c96d] px-5 py-3 text-sm font-semibold text-[#062112] shadow-[0_16px_28px_-17px_rgba(0,201,109,.78)] transition hover:-translate-y-0.5 hover:bg-[#00dc79] disabled:opacity-60">{saving ? label("Зберігаємо…", "Saving…") : label("Зберегти зміни", "Save changes")}</button>}</div></div>
    <div className="grid gap-6 lg:grid-cols-[.82fr_1.18fr]">
      <aside className="overflow-hidden rounded-[26px] bg-[#17251c] p-6 text-white shadow-[0_24px_55px_-35px_rgba(4,21,10,.85)] sm:p-7"><div className="flex items-start justify-between"><div className="relative"><div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl bg-[#eff5f0] text-3xl font-bold text-[#147b47]">{avatarUrl ? <img src={avatarUrl} alt="" width={80} height={80} loading="lazy" className="h-full w-full object-cover" /> : initials(user.username)}</div>{canEditProfile && <label className="absolute -bottom-2 -right-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg bg-[#00d977] text-[#062112] shadow-lg"><input className="sr-only" type="file" accept="image/png,image/jpeg,image/webp" onChange={onAvatar} /><Camera className="h-4 w-4" /></label>}</div><div className="rounded-xl bg-white/8 px-3 py-2 text-xs font-semibold text-[#b7c9ba]">ID {user.id > 0 ? user.id : "preview"}</div></div><h2 className="mt-7 text-2xl font-semibold tracking-[-.04em]">{user.firstName || user.username}</h2><p className="mt-1 text-sm text-[#a9bdaa]">{isEducational ? [roleLabel, className].filter(Boolean).join(" · ") : `${courseName} · ${label("особистий маршрут", "personal path")}`}</p><div className="mt-7 border-t border-white/10 pt-6"><div className="flex items-end justify-between"><div><div className="text-xs font-semibold uppercase tracking-[.14em] text-[#8ea992]">{isEducational ? label("Навчальний контекст", "Learning context") : label("Поточний рівень", "Current level")}</div><div className="mt-2 text-3xl font-semibold tracking-[-.06em] text-[#73ecae]">{isEducational ? (className || roleLabel) : currentIad}</div></div><div className="rounded-xl bg-[#00ff88]/12 p-2.5 text-[#73ecae]">{isEducational ? <GraduationCap className="h-5 w-5" /> : <Gauge className="h-5 w-5" />}</div></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#00d977]" style={{ width: isEducational ? "100%" : `${Math.min(100, currentIad)}%` }} /></div></div><div className="mt-7 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/[.07] p-4"><div className="text-2xl font-semibold">{stats.librarySolved}</div><div className="mt-1 text-xs leading-5 text-[#a9bdaa]">{label("задач розв’язано", "problems solved")}</div></div><div className="rounded-2xl bg-white/[.07] p-4"><div className="text-2xl font-semibold">{stats.avgGrade?.toFixed(0) ?? "—"}</div><div className="mt-1 text-xs leading-5 text-[#a9bdaa]">{label("середній результат", "average result")}</div></div></div></aside>
      <div className="space-y-6"><section className="rounded-[26px] border border-[#152219]/10 bg-white p-6 dark:border-white/10 dark:bg-[#121b15] sm:p-7"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-sm font-semibold text-[#147b47] dark:text-[#62ecaa]"><TrendingUp className="h-4 w-4" />{label("Поточний прогрес", "Current progress")}</div><h2 className="mt-3 text-2xl font-semibold tracking-[-.04em]">{label("Твоя динаміка", "Your momentum")}</h2><p className="mt-2 max-w-lg text-sm leading-6 text-[#6c7a70] dark:text-[#a4b2a7]">{label("Короткий підсумок практики, щоб бачити свій темп без зайвих налаштувань.", "A compact practice summary to help you see your pace without extra settings.")}</p></div><TrendingUp className="h-5 w-5 text-[#147b47] dark:text-[#62ecaa]" /></div><div className="mt-6 grid grid-cols-3 gap-2"><div className="rounded-xl bg-[#f5f8f5] px-3 py-4 dark:bg-white/[.04]"><div className="text-2xl font-semibold tracking-[-.04em]">{stats.totalGrades}</div><div className="mt-1 text-xs leading-5 text-[#718075] dark:text-[#9eada1]">{label("перевірок", "checks")}</div></div><div className="rounded-xl bg-[#f5f8f5] px-3 py-4 dark:bg-white/[.04]"><div className="text-2xl font-semibold tracking-[-.04em]">{stats.avgGrade?.toFixed(0) ?? "—"}</div><div className="mt-1 text-xs leading-5 text-[#718075] dark:text-[#9eada1]">{label("середній результат", "average result")}</div></div><div className="rounded-xl bg-[#f5f8f5] px-3 py-4 dark:bg-white/[.04]"><div className="text-2xl font-semibold tracking-[-.04em]">{weeklyActiveDays}/7</div><div className="mt-1 text-xs leading-5 text-[#718075] dark:text-[#9eada1]">{label("активних днів", "active days")}</div></div></div><div className="mt-5 flex items-center justify-between gap-3 rounded-xl bg-[#eaf9ef] px-4 py-3 dark:bg-[#00ff88]/[.07]"><span className="text-sm font-semibold text-[#147b47] dark:text-[#72edb0]">{label("Поточний фокус", "Current focus")}</span><span className="rounded-lg bg-white/70 px-2.5 py-1 text-xs font-bold text-[#526157] dark:bg-white/[.08] dark:text-[#c7d5ca]">{contextLabel}</span></div>{message && <p className="mt-4 text-sm font-medium text-[#147b47] dark:text-[#62ecaa]">{message}</p>}</section>
...
export const PremiumLibrary: React.FC<{
  ...
  getTaskHref?: (task: LibraryTaskListItem) => string;
}> = ({ tasks, total, solved, loading, query, onQuery, onOpen, getTaskHref }) => {
  const { i18n } = useTranslation();
  const en = i18n.language?.toLowerCase().startsWith("en");
  ...
  return <div className="min-h-full bg-[#f7f8f5] px-4 py-6 text-[#142017] dark:bg-[#0b120e] dark:text-[#edf3ef] sm:px-6 lg:px-10 lg:py-9"><div className="mx-auto max-w-7xl space-y-6 lg:space-y-8">
    ...
    {loading ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" role="status" aria-label={en ? "Loading task library" : "Завантаження бібліотеки задач"}>{Array.from({ length: 6 }, (_, index) => <div key={index} className="h-64 animate-pulse rounded-[24px] bg-[#e9eeea] dark:bg-white/[.045]" />)}</div> : filteredTasks.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filteredTasks.map((task) => <article key={task.id} className="group flex min-h-[250px] flex-col rounded-[24px] border border-[#152219]/10 bg-white p-5 shadow-[0_16px_38px_-34px_rgba(17,43,25,.45)] transition hover:-translate-y-1 hover:border-[#00c96d]/40 hover:shadow-[0_20px_45px_-30px_rgba(17,58,31,.42)] dark:border-white/10 dark:bg-[#121b15] dark:hover:bg-[#152019]"><div className="flex items-start justify-between gap-3"><div className="flex flex-wrap gap-2"><span className={`rounded-lg px-2.5 py-1 text-xs font-semibold ${tone(task.difficulty)}`}>{difficulty(task.difficulty)}</span>{task.projectSpec ? <span className="inline-flex items-center gap-1 rounded-lg bg-[#fff4df] px-2.5 py-1 text-xs font-semibold text-[#a65600] dark:bg-[#ffb454]/10 dark:text-[#ffca7e]"><Rocket className="h-3 w-3" aria-hidden="true" />{copy.project}</span> : null}</div><span className="text-xs font-medium text-[#809084] dark:text-[#96a69b]">{task.taskMode === "WEB" ? copy.web : copy.code}</span></div><h2 className="mt-5 text-lg font-semibold tracking-tight">{task.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-[#6b7b70] dark:text-[#a0aea3]">{task.description || task.section || "Practice the concept in your own solution."}</p>{task.attempt?.lastCheckedAt ? <p className="mt-2 text-[11px] font-medium text-[#147b47] dark:text-[#72edb0]">{en ? "Checked" : "Перевірено"}: {new Intl.DateTimeFormat(i18n.language || "uk", { dateStyle: "short", timeStyle: "short" }).format(new Date(task.attempt.lastCheckedAt))}</p> : null}<div className="mt-auto flex items-center justify-between gap-3 pt-5"><div className="flex flex-wrap gap-1.5">{(task.tags || []).slice(0, 2).map((tag) => <span key={tag} className="rounded-md bg-[#f3f6f3] px-2 py-1 text-[11px] font-medium text-[#607064] dark:bg-white/[.06] dark:text-[#a4b2a8]">{tag}</span>)}</div><div className="flex items-center gap-2">{getTaskHref ? <a href={getTaskHref(task)} target="_blank" rel="noopener noreferrer" className="rounded-lg p-2 text-[#718075] transition hover:bg-[#f0f4ef] hover:text-[#147b47] dark:hover:bg-white/[.06] dark:hover:text-[#72edb0]" aria-label={en ? "Open task in a new tab" : "Відкрити задачу в новій вкладці"} title={en ? "Open in a new tab" : "Відкрити в новій вкладці"}><ExternalLink className="h-4 w-4" /></a> : null}<button type="button" onClick={() => onOpen(task)} className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#147b47] transition group-hover:text-[#00a75a] dark:text-[#62ecaa]">{copy.open}<ArrowRight className="h-4 w-4" aria-hidden="true" /></button></div></div></article>)}</div> : <div className="rounded-[26px] border border-dashed border-[#152219]/15 bg-white px-5 py-16 text-center text-sm text-[#718075] dark:border-white/10 dark:bg-[#121b15] dark:text-[#9caea0]" role="status"><p>{copy.empty}</p>{(query || difficultyFilter !== "ALL" || modeFilter !== "ALL") && <button type="button" onClick={clearSearch} className="mt-4 rounded-xl bg-[#17251c] px-4 py-2.5 text-xs font-bold text-white dark:bg-[#00d978] dark:text-[#062211]">{en ? "Reset filters" : "Скинути фільтри"}</button>}</div>}
  </div></div>;
};
