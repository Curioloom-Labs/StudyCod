    ...
    <div className="grid min-h-[610px] grid-cols-[174px_1fr] max-md:block max-md:min-h-0">
      ...
      <div className="min-w-0 bg-[#f7f8f5] p-7 max-md:p-4">
        ...
        <div className="grid min-h-[150px] grid-cols-[138px_1fr_auto] items-center gap-5 rounded-[19px] border border-[#122017]/10 bg-white p-3.5 shadow-[0_13px_35px_rgba(18,32,23,0.05)] max-md:grid-cols-[92px_1fr] max-md:gap-3">
          ...
          <button type="button" className="grid size-10 place-items-center rounded-xl bg-[#00ff88] text-[#06150d] max-md:hidden" aria-label={tr("Продовжити урок", "Continue lesson")}><Play className="size-4 fill-current" /></button>
        </div>
  ...
  <div className={`${card} overflow-hidden`}>
    ...
    <div className="grid min-h-[430px] grid-cols-[1.5fr_.72fr] border-t border-[#122017]/10 max-md:grid-cols-1">
      <div className="flex flex-col bg-[#101713] text-[#dbe7df]">
        ...
        <div className="flex justify-end border-t border-white/10 p-4"><button type="button" className="flex h-9 items-center gap-2 rounded-[10px] bg-[#00ff88] px-3.5 text-[10px] font-bold text-[#06150d]"><Play className="size-3 fill-current" />{tr("Перевірити рішення", "Check solution")}</button></div>
      </div>
      ...
      <main>
        <section className="relative mx-auto w-[min(1240px,calc(100%_-_48px))] pb-20 pt-[158px] text-center max-md:w-[calc(100%_-_28px)] max-md:pb-14 max-md:pt-[116px]">
          ...
          <motion.div className="relative z-10 mx-auto max-w-[900px]" initial={reduceMotion ? undefined : { opacity: 0, y: 24 }} animate={reduceMotion ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.75, ease: easing }}>
            ...
            <div className="mb-5 flex items-center justify-center gap-2 text-[10px] font-extrabold uppercase tracking-[.18em] text-[#007f48] dark:text-[#68efb0]"><span className="h-px w-8 bg-[#00b963]/40" />StudyCod — learn by building<span className="h-px w-8 bg-[#00b963]/40" /></div>
         ...
         <section id="proof" className="mx-auto w-[min(1120px,calc(100%_-_48px))] pb-10 pt-[130px] max-md:w-[calc(100%_-_28px)] max-md:pt-24">
           ...
           <div className="grid gap-[18px] lg:grid-cols-[.86fr_1.14fr]">
             <motion.article key="learner-proof" {...reveal} className={`${card} p-7 sm:p-8`}><div className="flex items-center gap-3 text-[11px] font-extrabold uppercase tracking-[.1em] text-[#007f48]"><span className={iconTile}><GraduationCap className="size-5" /></span>{tr("Доказ учня", "Learner proof")}</div><h3 className="mt-7 text-[28px] font-bold leading-[1.1] tracking-[-.04em]">{tr("Не просто правильна відповідь — подолана помилка.", "Not just a correct answer — an overcome mistake.")}</h3><p className="mt-4 text-[15px] leading-7 text-[#667169]">{tr("Кожна здача залишає зрозумілий слід: що не пройшло, яка підказка допомогла і що вдалося виправити.", "Every submission leaves a clear trail: what failed, which hint helped, and what was fixed.")}</p><div className="mt-7 space-y-2 rounded-2xl bg-[#f5f8f5] p-4 dark:bg-white/[.04]">{[tr("4 / 5 тестів пройдено", "4 / 5 tests passed"), tr("Помилка: межа діапазону", "Failure: boundary condition"), tr("Підказка 2 з 3 → повторна спроба", "Hint 2 of 3 → retry"), tr("Навичку закріплено", "Skill reinforced")].map((item, index) => <div key={item} className="flex items-center gap-2 text-xs font-semibold text-[#526157] dark:text-[#c1cdc4]"><span className={`grid size-5 place-items-center rounded-full text-[10px] ${index === 3 ? "bg-[#00d978] text-[#062211]" : "bg-[#dceee1] text-[#147b47] dark:bg-[#00ff88]/10 dark:text-[#72edb0]"}`}>{index === 3 ? "✓" : index + 1}</span>{item}</div>)}</div></motion.article>
             <motion.article key="teacher-proof" {...reveal} className={`${card} bg-[linear-gradient(145deg,#fff,rgba(255,180,84,.08))] p-7 sm:p-8 dark:bg-[linear-gradient(145deg,#151c17,rgba(255,180,84,.08))]`}><div className="mb-5 flex items-center justify-between gap-3"><div className="flex items-center gap-3 text-[11px] font-extrabold uppercase tracking-[.1em] text-[#ad6900]"><span className={`${iconTile} bg-[#ffb454]/10 text-[#ad6900]`}><Users className="size-5" /></span>{tr("Доказ викладача", "Teacher proof")}</div><span className="rounded-full bg-[#00ff88]/10 px-2.5 py-1 text-[10px] font-bold text-[#147b47]">{tr("У реальному часі", "Real time")}</span></div><h3 className="text-[28px] font-bold leading-[1.1] tracking-[-.04em]">{tr("Не лише оцінка — карта того, де класу потрібна допомога.", "Not just a score — a map of where the class needs help.")}</h3><p className="mt-4 text-[15px] leading-7 text-[#667169]">{tr("Поки йде live-урок, викладач бачить статуси, повторювані помилки та наступну дію.", "During a live lesson, teachers see statuses, recurring errors, and the next action.")}</p><div className="mt-7"><TeacherRadarPreview tr={tr} /></div></motion.article>
           </div>
         </section>
