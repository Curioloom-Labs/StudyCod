...
export const HomePage: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener("studycod:resumeState", handler as any);
    return () => window.removeEventListener("studycod:resumeState", handler as any);
  }, [user.id]);
  useEffect(() => {
    if (!isEducational) {
      setLoading(true);
  ...
  const averageGradeData = useMemo(() => {
    ...
    const controlTotals = (studentSummaryGrades || []).filter((sg: any) => sg && sg.assessmentType === "CONTROL").filter((sg: any) => sg.controlWorkId && controlIds.has(sg.controlWorkId)).map((sg: any) => ({
      grade: sg.grade,
      label: sg.controlWorkTitle || sg.name || tr("Контрольна", "Control work")
    })).filter((x: any) => typeof x.grade === "number" && x.grade > 0) as Array<{
      grade: number;
      label: string;
    }>;
  ...
  const nextStudentLesson = useMemo(() => {
    ...
    const hasActiveTasks = (lesson: Lesson) => {
      const tasks = (lesson.tasks || []) as any[];
    ...
    const isActionable = (lesson: Lesson) => {
      if (lesson.deadline && isDeadlineExpired(lesson.deadline)) {
        // If the lesson is past deadline but it contains any still-active tasks, keep it actionable.
  ...
  if (ui.mode === "focus") {
    if (focusResumeActive) {
      // Avoid any flash of an entry screen; navigation is handled in the effect above.
      return null;
    }
