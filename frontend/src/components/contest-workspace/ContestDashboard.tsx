      ...
      <div className="xl:col-span-4 min-h-0 flex flex-col gap-3">
        <div className="rounded-2xl border border-border/70 bg-bg-surface/85 p-4 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
          ...
          <div className="mt-4 flex items-center justify-center">
            <svg width="130" height="130" viewBox="0 0 130 130" className="drop-shadow-[0_0_20px_rgba(98,149,255,0.26)]">
              <circle cx="65" cy="65" r={radius} fill="none" stroke="#2a2a3a" strokeWidth="10" />
              <circle
                cx="65"
                cy="65"
                r={radius}
              ...
              />
              <text x="65" y="62" textAnchor="middle" className="fill-text-secondary" style={{ fontSize: 10 }}>remaining</text>
          ...
          <div className="mt-2 text-xs text-text-secondary flex items-center justify-between">
            <span>{started ? (finished ? "Finished" : "Running") : "Upcoming"}</span>
          ...
          <div className="mt-3 flex items-center gap-2 text-xs">
            ...
            <span className={freeze ? "text-accent-warn" : "text-text-secondary"}>
              {freeze ? "Freeze period active" : "No freeze right now"}
            </span>
          </div>
          ...
          <div className="mt-2 flex-1 min-h-0 overflow-auto space-y-2">
            ...
            {activity.map((r) => (
              <div key={r.participantId} className="rounded-lg border border-border bg-bg-base/70 p-2 text-xs">
                ...
                <div className="text-text-secondary">improved total to {r.totalScore}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="w-full min-w-[480px] text-xs border border-border rounded-lg overflow-hidden">
            <thead className="bg-bg-hover">
              <tr>
                ...
                <th className="p-2 border-b border-border text-left">Participant</th>
                <th className="p-2 border-b border-border text-right">Total</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.participantId} className={`${me ? "bg-secondary/10" : "odd:bg-bg-base even:bg-bg-surface"}`}>
                    ...
                    <td className="p-2 border-b border-border text-text-primary">
                      {r.displayName}
                      {me ? <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-secondary/20 text-secondary">you</span> : null}
                    </td>
