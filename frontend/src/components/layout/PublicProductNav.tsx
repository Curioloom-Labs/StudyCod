...
export const PublicProductNav: React.FC<Props> = ({ active = "none", homeMode = false }) => {
  ...
  return <><a className="skip-link" href="#main-content">{tr("Перейти до основного вмісту", "Skip to main content")}</a><header className={(homeMode ? "fixed" : "sticky") + " inset-x-0 top-0 z-50 h-[74px] border-b border-[#122017]/[.07] bg-[#f7f8f5]/85 backdrop-blur-xl dark:border-white/10 dark:bg-[#0d120f]/85 max-md:h-16"}>
    <div className="mx-auto flex h-full w-[min(1240px,calc(100%_-_48px))] items-center justify-between gap-7 max-md:w-[calc(100%_-_28px)] max-md:gap-2">
      ...
      <nav className="flex items-center gap-7 max-[980px]:hidden" aria-label={tr("Головна навігація", "Main navigation")}>
        {homeMode ? homeItems.map(item => <button type="button" key={item.id} onClick={() => goHomeItem(item.id)} className="relative py-2 text-[13px] font-semibold text-[#667169] transition after:absolute after:inset-x-0 after:bottom-0 after:h-px after:origin-left after:scale-x-0 after:bg-[#00b963] after:transition-transform hover:text-[#111814] hover:after:scale-x-100 dark:text-[#9da9a1] dark:hover:text-white">{item.label}</button>) : routeItems.map(item => <a key={item.id} href={item.path} className={"relative py-2 text-[13px] font-semibold transition after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-[#00b963] " + (active === item.id ? "text-[#111814] after:scale-x-100 dark:text-white" : "text-[#667169] after:scale-x-0 hover:text-[#111814] hover:after:scale-x-100 dark:text-[#9da9a1] dark:hover:text-white")}>{item.label}</a>)}
      </nav>

      <div className="flex items-center gap-2">
        <button type="button" onClick={() => i18n.changeLanguage(i18n.language === "uk" ? "en" : "uk")} aria-label={i18n.language === "uk" ? tr("Перемкнути на англійську", "Switch to English") : tr("Перемкнути на українську", "Switch to Ukrainian")} className="flex h-10 items-center gap-1.5 rounded-xl border border-[#122017]/10 bg-white px-3 text-[11px] font-bold text-[#667169] transition hover:border-[#00b963]/30 hover:text-[#111814] dark:border-white/10 dark:bg-[#182019] dark:text-[#a7b2aa] dark:hover:text-white max-md:w-9 max-md:justify-center max-md:px-0"><Languages className="size-3.5" aria-hidden="true" /><span className="max-md:hidden">{i18n.language === "uk" ? "EN" : "UA"}</span></button>
